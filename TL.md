...
- **Expression-AST variants hold payloads by value, not `&'t T`** — contradicts TFITCX (`/// Arena-allocated` ⇒ `&'t T` access) and design v3 §7.1 ("sub-expressions are `&'t` refs"). All 52 variants (47 `ReferenceExpressionTE` + 5 `AddressExpressionTE`) and ~304 use sites across 28 files would flip; surfaced via `load_from_static_sized_array` SPDMX escalation (in-file precedent `load_from_struct` wraps inside the fn as workaround). Deferred until post-Slab 15.
