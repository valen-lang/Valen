...
**Per-TL CI loop (after every green test, from your own worktree):** `git rebase experimental` → re-verify green → **pause and wait for the architect's explicit go-ahead before syncing** → `git fetch . experimental-N:experimental` to fast-forward `experimental` to your tip. Check the exit code; non-ff rejection means another TL beat you — re-rebase and retry. The architect gate before the fetch is non-negotiable: never sync to `experimental` unilaterally. Concurrency-safe via git's ref locking. No remote pushes during the sprint; all worktrees share `/Volumes/V/Vale/.git` so branches are visible by name from anywhere.
