  ...
  buildIfNever(
      ...
      [globalState, failMessage](LLVMBuilderRef bodyBuilder) {
        if (globalState->opt->includeBoundsChecks) {
          ...
          buildCallWith64BitSExt(globalState, bodyBuilder, globalState->externs->exit, {constI64LE(globalState, 1)});
        }
...
InBoundsLE checkLastElementExists(
    ...
    Ref sizeRef,
    const char* failMessage) {
  auto intRefMT = globalState->metalCache->i32Type;
  auto sizeLE =
...
void intRangeLoopV(
  ...
  intRangeLoop(
      ...
      [globalState, iterationBuilder](LLVMValueRef iterationIndexLE, LLVMBuilderRef builder) {
        auto iterationIndexRef = toRef(globalState->getRegion(globalState->metalCache->i32Type), globalState->metalCache->i32Type, iterationIndexLE);
...
void intRangeLoop(
  ...
  buildWhile(
      ...
      [globalState, iterationBuilder, int32LT, iterationIndexPtrLE](LLVMBuilderRef bodyBuilder) {
        ...
        adjustCounterV(
            globalState, bodyBuilder, globalState->metalCache->i32Type, iterationIndexPtrLE, 1, false);
      });
}
