#!/usr/bin/env python3
"""
Generate implementation-only Scala->Rust mapping for the postparser migration.

This is a postparser-specific wrapper around check_scala_rust_mapping.py that:
  - scans Scala postparsing sources
  - skips Scala test files/paths
  - searches Rust postparsing implementation (excluding Rust tests by default)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Check postparser Scala->Rust mappings (implementation only)"
    )
    parser.add_argument(
        "--scala-dir",
    ...
    parser.add_argument(
        ...
        / ".cursor"
        / "rules"
        / "postparser_impl_scala_rust_mapping.mdc",
        type=Path,
    ...
    with output_path.open("w", encoding="utf-8") as f:
        f.write("---\n")
        f.write("description: Postparser Scala-to-Rust implementation mapping\n")
        f.write("globs: FrontendRust/src/postparsing/**/*.rs\n")
        f.write("alwaysApply: false\n")
        f.write("---\n\n")
        f.write("# Postparser Implementation Mapping (Scala -> Rust)\n\n")
