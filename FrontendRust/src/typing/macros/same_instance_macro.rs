    ...
    pub fn generate_function_body_same_instance(
        ...
        let body = ReferenceExpressionTE::Block(BlockTE {
            inner: self.typing_interner.alloc(ReferenceExpressionTE::Return(ReturnTE {
                source_expr: self.typing_interner.alloc(ReferenceExpressionTE::IsSameInstance(IsSameInstanceTE {
                    left: self.typing_interner.alloc(ReferenceExpressionTE::ArgLookup(ArgLookupTE {
                        param_index: 0,
                        coord: param_coords[0].tyype,
                    })),
                    right: self.typing_interner.alloc(ReferenceExpressionTE::ArgLookup(ArgLookupTE {
                        param_index: 1,
