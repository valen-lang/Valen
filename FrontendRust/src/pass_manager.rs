...
fn resolve_package_contents(
    ...
    for (index, input) in inputs.iter().enumerate() {
        ...
        match input {
            IFrontendInput::SourceInput { package_coord: _, name, code } => {
            ...
            IFrontendInput::ModulePathInput { module: _, module_path } => {
            ...
            IFrontendInput::DirectFilePathInput { package_coord: _, path } => {
...
pub fn build(
    ...
    use crate::passmanager::{FullCompilationOptions, GlobalOptions};
    ...
    use crate::passmanager::FullCompilation;
    ...
    let parseds = match compilation.get_parseds() {
        Err(failed_parse) => {
            ...
            panic!("ParseErrorHumanizer.humanize not yet implemented. FailedParse: {:?}", failed_parse);
        }
        Ok(p) => p,
    };
    ...
    if opts.output_vpst {
        ...
        for (file_coord, (program_p, _comment_ranges)) in &parseds.file_coord_to_contents {
            ...
            std::fs::write(&vpst_filepath, vpst_json)
    ...
    if opts.benchmark {
        println!("Loading and parsing duration: {:?}", _start_scout_time.duration_since(_start_load_and_parse_time));
    }
