...
use crate::utils::code_hierarchy::{FileCoordinate, PackageCoordinate, IPackageResolver, FileCoordinateMap};
    ...
    R: IPackageResolver<HashMap<String, String>>,
{
    ...
    while !unexplored_packages.is_empty() {
        ...
        let filepaths_map: HashMap<Arc<FileCoordinate>, String> = 
            filepaths_and_contents.iter().map(|(fc, code)| (fc.clone(), code.clone())).collect();
    ...
    R: IPackageResolver<HashMap<String, String>>,
{
    ...
    lex_and_explore(
        ...
        |file_coord, code, imports, denizen| {
            denizens.push((file_coord.clone(), code.to_string(), imports.to_vec(), denizen.clone()));
            denizen.clone()
        },
