...
fn test_templated_struct_one_arg() {
    ...
    if let IRulexPR::Templex(ITemplexPT::Call {
        template: box ITemplexPT::NameOrRune(NameP { str: ref moo_str, .. }),
        args: ref args,
        ..
    }) = result1 {
        assert_eq!(moo_str.str, "Moo");
        assert_eq!(args.len(), 1);
        assert!(matches!(&args[0], ITemplexPT::NameOrRune(NameP { str: ref s, .. }) if s.str == "int"));
    } else {
    ...
    if let IRulexPR::Templex(ITemplexPT::Call {
        ...
        assert_eq!(moo_str.str, "Moo");
        assert_eq!(args.len(), 1);
        assert!(matches!(&args[0], ITemplexPT::Interpreted {
            maybe_ownership: Some(box OwnershipPT { ownership: OwnershipP::Share, .. }),
            maybe_region: None,
            inner: box ITemplexPT::NameOrRune(NameP { str: ref s, .. }),
            ..
        } if s.str == "int"));
    } else {
        panic!("Expected Templex with Call");
...
fn test_rwkilc() {
    ...
    if let IRulexPR::Templex(ITemplexPT::Call {
        template: box ITemplexPT::NameOrRune(NameP { str: ref k_str, .. }),
        args: ref args,
        ..
    }) = result3 {
...
fn test_templated_struct_rune_arg() {
    ...
    if let IRulexPR::Templex(ITemplexPT::Call {
        ...
        assert_eq!(moo_str.str, "Moo");
        assert_eq!(args.len(), 1);
        assert!(matches!(&args[0], ITemplexPT::NameOrRune(NameP { str: ref s, .. }) if s.str == "R"));
    } else {
        panic!("Expected Templex with Call");
    }
}
...
fn test_templated_struct_multiple_args() {
    ...
    if let IRulexPR::Templex(ITemplexPT::Call {
        ...
        assert!(matches!(&args[0], ITemplexPT::NameOrRune(NameP { str: ref s, .. }) if s.str == "int"));
        assert!(matches!(&args[1], ITemplexPT::NameOrRune(NameP { str: ref s, .. }) if s.str == "str"));
    } else {
        panic!("Expected Templex with Call");
    }
}
...
fn test_templated_struct_arg_is_another_templated_struct_with_one_arg() {
    ...
    if let IRulexPR::Templex(ITemplexPT::Call {
        ...
        if let ITemplexPT::Call {
            template: box ITemplexPT::NameOrRune(NameP { str: ref blarg_str, .. }),
            args: ref inner_args,
            ..
        } = &args[0] {
            assert_eq!(blarg_str.str, "Blarg");
            assert_eq!(inner_args.len(), 1);
            assert!(matches!(&inner_args[0], ITemplexPT::NameOrRune(NameP { str: ref s, .. }) if s.str == "int"));
        } else {
...
fn test_templated_struct_arg_is_another_templated_struct_with_multiple_arg() {
    ...
    if let IRulexPR::Templex(ITemplexPT::Call {
        ...
        if let ITemplexPT::Call {
            ...
            assert!(matches!(&inner_args[0], ITemplexPT::NameOrRune(NameP { str: ref s, .. }) if s.str == "int"));
            assert!(matches!(&inner_args[1], ITemplexPT::NameOrRune(NameP { str: ref s, .. }) if s.str == "str"));
        } else {
            panic!("Expected inner Call");
        }
    } else {
...
fn test_static_sized_array() {
    ...
    if let ITemplexPT::StaticSizedArray {
        ...
        size: box ITemplexPT::AnonymousRune(_),
        element: box ITemplexPT::AnonymousRune(_),
        ..
    } = result1 {
        // Success
    } else {
    ...
    if let ITemplexPT::StaticSizedArray {
        ...
        element: box ITemplexPT::NameOrRune(NameP { str: ref s, .. }),
        ..
    } = result3 {
        assert_eq!(s.str, "int");
    } else {
    ...
    if let ITemplexPT::StaticSizedArray {
        mutability: box ITemplexPT::Mutability { mutability: MutabilityP::Mutable, .. },
        variability: box ITemplexPT::Variability { variability: VariabilityP::Final, .. },
        size: box ITemplexPT::NameOrRune(NameP { str: ref n_str, .. }),
    ...
    if let ITemplexPT::StaticSizedArray {
        ...
        variability: box ITemplexPT::Variability { variability: VariabilityP::Final, .. },
        size: box ITemplexPT::NameOrRune(NameP { str: ref n_str, .. }),
        element: box ITemplexPT::NameOrRune(NameP { str: ref t_str, .. }),
        ..
    } = result6 {
        assert_eq!(n_str.str, "N");
        assert_eq!(t_str.str, "T");
...
fn test_regular_sequence() {
    ...
    if let ITemplexPT::Tuple { elements: ref elems, .. } = result1 {
    ...
    if let ITemplexPT::Tuple { elements: ref elems, .. } = result2 {
    ...
    if let ITemplexPT::Tuple { elements: ref elems, .. } = result3 {
        assert_eq!(elems.len(), 2);
        assert!(matches!(&elems[0], ITemplexPT::NameOrRune(NameP { str: ref s, .. }) if s.str == "int"));
        assert!(matches!(&elems[1], ITemplexPT::NameOrRune(NameP { str: ref s, .. }) if s.str == "bool"));
    } else {
    ...
    if let ITemplexPT::Tuple { elements: ref elems, .. } = result4 {
        assert_eq!(elems.len(), 2);
        assert!(matches!(&elems[0], ITemplexPT::AnonymousRune(_)));
        assert!(matches!(&elems[1], ITemplexPT::NameOrRune(NameP { str: ref s, .. }) if s.str == "bool"));
    } else {
    ...
    if let ITemplexPT::Tuple { elements: ref elems, .. } = result5 {
        assert_eq!(elems.len(), 2);
        assert!(matches!(&elems[0], ITemplexPT::AnonymousRune(_)));
        assert!(matches!(&elems[1], ITemplexPT::AnonymousRune(_)));
    } else {
...
fn test_prototype_kind_rule() {
    ...
    if let ITemplexPT::Func {
        name: NameP { str: ref moo_str, .. },
        parameters: ref params,
        return_type: box ITemplexPT::NameOrRune(NameP { str: ref void_str, .. }),
        ..
    } = result1 {
        assert_eq!(moo_str.str, "moo");
        assert_eq!(params.len(), 1);
        assert!(matches!(&params[0], ITemplexPT::NameOrRune(NameP { str: ref s, .. }) if s.str == "int"));
    ...
    if let ITemplexPT::Func {
        name: NameP { str: ref moo_str, .. },
        parameters: ref params,
        return_type: box ITemplexPT::NameOrRune(NameP { str: ref r_str, .. }),
        ..
    } = result2 {
        assert_eq!(moo_str.str, "moo");
        assert_eq!(params.len(), 1);
        assert!(matches!(&params[0], ITemplexPT::NameOrRune(NameP { str: ref s, .. }) if s.str == "T"));
