...
use crate::parsing::ast::FunctionHeaderP;
use crate::parsing::ast::{FileP, IDenizenP, IAttributeP, IExpressionPE, IRulexPR, ITemplexPT, PatternPP, StructP};
use crate::parsing::ast::GenericParametersP;
use crate::parsing::ast::TemplateRulesP;
use crate::parsing::ast::IStructContent;
use crate::parsing::ast::NormalStructMemberP;
use crate::parsing::ast::VariadicStructMemberP;
use crate::parsing::ast::RegionRunePT;
use crate::parsing::ast::FunctionReturnP;
...
trait VisitorP {
  fn visit_denizen_recurse(&self, denizen: &IDenizenP) -> u64 {
    match denizen {
      IDenizenP::TopLevelFunction(function) => self.visit_function_recurse(function),
      IDenizenP::TopLevelStruct(struct_) => self.visit_struct_recurse(struct_),
      IDenizenP::TopLevelInterface(interface) => self.visit_interface_recurse(interface),
      IDenizenP::TopLevelImpl(impl_) => self.visit_impl_recurse(impl_),
      IDenizenP::TopLevelExportAs(export_as) => self.visit_export_as_recurse(export_as),
      IDenizenP::TopLevelImport(import) => self.visit_import_recurse(import),
    }
  }

  fn visit_struct_recurse(&self, struct_: &StructP) -> u64 {
    let mut count = 0;
    count += if self.visit_struct(struct_) { 1 } else { 0 };
    let StructP { range, name, attributes, mutability, identifying_runes, template_rules, maybe_default_region_rune, body_range, members } = struct_;
    for attribute in attributes {
      count += self.visit_attribute_recurse(attribute);
    }
    if let Some(identifying_runes) = identifying_runes {
      count += self.visit_generic_parameters_recurse(identifying_runes);
    }
    if let Some(template_rules) = template_rules {
      count += self.visit_template_rules_recurse(template_rules);
    }
    if let Some(maybe_default_region_rune) = maybe_default_region_rune {
      count += self.visit_region_rune_recurse(maybe_default_region_rune);
    }
    for member in &members.contents {
      count += self.visit_struct_member_recurse(member);
    }
    count
  }
  fn visit_struct(&self, _struct_: &StructP) -> bool {
    return false;
  }

  fn visit_struct_member_recurse(&self, member: &IStructContent) -> u64 {
    match member {
      IStructContent::StructMethod(function) => self.visit_function_recurse(function),
      IStructContent::NormalStructMember(normal_member) => self.visit_normal_struct_member_recurse(normal_member),
      IStructContent::VariadicStructMember(variadic_member) => self.visit_variadic_struct_member_recurse(variadic_member),
    }
  }

  fn visit_normal_struct_member_recurse(&self, member: &NormalStructMemberP) -> u64 {
    let mut count = 0;
    count += if self.visit_normal_struct_member(member) { 1 } else { 0 };
    let NormalStructMemberP { range, name, variability, tyype } = member;
    count += self.visit_name_recurse(name);
    count += self.visit_templex_recurse(tyype_member);
    count
  }
  fn visit_normal_struct_member(&self, _member: &NormalStructMemberP) -> bool {
    return false;
  }

  fn visit_variadic_struct_member_recurse(&self, member: &VariadicStructMemberP) -> u64 {
    let mut count = 0;
    count += if self.visit_variadic_struct_member(member) { 1 } else { 0 };
    let VariadicStructMemberP { range, variability, tyype } = member;
    count += self.visit_templex_recurse(tyype);
    count
  }
  fn visit_variadic_struct_member(&self, _member: &VariadicStructMemberP) -> bool {
    return false;
  }

  fn visit_interface_recurse(&self, interface: &InterfaceP) -> u64 {
    let mut count = 0;
    count += if self.visit_interface(interface) { 1 } else { 0 };
    let InterfaceP { range, name, attributes, mutability, maybe_identifying_runes, template_rules, maybe_default_region_rune, body_range, members } = interface;
    for attribute in attributes {
      count += self.visit_attribute_recurse(attribute);
    }
    if let Some(maybe_identifying_runes) = maybe_identifying_runes {
      count += self.visit_generic_parameters_recurse(maybe_identifying_runes);
    }
    if let Some(template_rules) = template_rules {
      count += self.visit_template_rules_recurse(template_rules);
    }
    ...
    for member in members {
      count += self.visit_function_recurse(member);
    }
    count
  }
  fn visit_interface(&self, _interface: &InterfaceP) -> bool {
    return false;
  }

  fn visit_function_recurse(&self, function: &FunctionP) -> u64 {
    let mut count = 0;
    // Visit the function itself
    count += if self.visit_function(function) { 1 } else { 0 };
    // Recurse down into function's fields
    let FunctionP { range, header, body } = function;
    count += self.visit_function_header_recurse(header);
    if let Some(body) = body {
      count += self.visit_block_recurse(body);
    }
    count
  }
  fn visit_function(&self, _function: &FunctionP) -> bool {
    return false;
  }

  fn visit_function_header_recurse(&self, header: &FunctionHeaderP) -> u64 {
    let mut count = 0;
    count += if self.visit_function_header(header) { 1 } else { 0 };
    let FunctionHeaderP { range, name, params, ret, generic_parameters, template_rules, attributes } = header;
    count += self.visit_function_return_recurse(ret);
    if let Some(generic_parameters) = generic_parameters {
      count += self.visit_generic_parameters_recurse(generic_parameters);
    }
    if let Some(template_rules) = template_rules {
      count += self.visit_template_rules_recurse(template_rules);
    }
  ...
  fn visit_function_header(&self, _header: &FunctionHeaderP) -> bool {
    return false;
  }

  fn visit_function_return_recurse(&self, return_: &FunctionReturnP) -> u64 {
    let mut count = 0;
    count += if self.visit_function_return(return_) { 1 } else { 0 };
    let FunctionReturnP { range, ret_type } = return_;
    if let Some(ret_type) = ret_type {
      count += self.visit_templex_recurse(ret_type);
    }
    count
  }
  fn visit_function_return(&self, _return_: &FunctionReturnP) -> bool {
    return false;
  }

  fn visit_generic_parameters_recurse(&self, generic_parameters: &GenericParametersP) -> u64 {
    let mut count = 0;
    count += if self.visit_generic_parameters(generic_parameters) { 1 } else { 0 };
    let GenericParametersP { range, params } = generic_parameters;
    for param in params {
      count += self.visit_generic_parameter_recurse(param);
    }
    count
  }
  fn visit_generic_parameters(&self, _generic_parameters: &GenericParametersP) -> bool {
    return false;
  }

  fn visit_template_rules_recurse(&self, template_rules: &TemplateRulesP) -> u64 {
    let mut count = 0;
    count += if self.visit_template_rules(template_rules) { 1 } else { 0 };
    let TemplateRulesP { range, rules } = template_rules;
    for rule in rules {
      count += self.visit_rulex_recurse(rule);
    }
    count
  }
  fn visit_template_rules(&self, _template_rules: &TemplateRulesP) -> bool {
    return false;
  }

  fn visit_region_rune_recurse(&self, region_rune: &RegionRunePT) -> u64 {
    let mut count = 0;
    count += if self.visit_region_rune(region_rune) { 1 } else { 0 };
  ...
  fn visit_region_rune(&self, _region_rune: &RegionRunePT) -> bool {
    return false;
  }

  fn visit_attribute_recurse(&self, attribute: &IAttributeP) -> u64 {
    let mut count = 0;
    count += if self.visit_attribute(attribute) { 1 } else { 0 };
    match attribute {
      IAttributeP::WeakableAttribute(range) => count += self.visit_weakable_attribute_recurse(range),
      IAttributeP::SealedAttribute(range) => count += self.visit_sealed_attribute_recurse(range),
      IAttributeP::MacroCall(range, inclusion, name) => count += self.visit_macro_call_recurse(range, inclusion, name),
    }
    count
  }


  fn visit_expression(&self, expression: &IExpressionPE) -> u64;
  fn visit_pattern(&self, pattern: &PatternPP) -> u64;
  fn visit_templex(&self, templex: &ITemplexPT) -> u64 {
    match templex {
      
    }
  }
  fn visit_rulex(&self, rulex: &IRulexPR) -> u64;
}
