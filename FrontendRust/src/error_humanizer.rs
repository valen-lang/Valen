// Mirrors Frontend/ParsingPass/src/dev/vale/parsing/ParseErrorHumanizer.scala
...
impl ParseErrorHumanizer {
    pub fn humanize(file_path: &Path, source: &str, err: &ParseError) -> String {
        ...
        let pos_str = Self::humanize_pos(file_path, source, err.pos());
        let next_stuff = Self::next_thing_and_rest_of_line(source, err.pos() as usize);
        
        format!("{} error {}: {}\n{}\n", pos_str, err.error_id(), error_message, next_stuff)
    }
    
    // Mirrors SourceCodeUtils.scala:humanizePos
    fn humanize_pos(file_path: &Path, source: &str, pos: i32) -> String {
        let mut line = 0;
    ...
    fn next_thing_and_rest_of_line(source: &str, pos: usize) -> String {
    ...
    fn get_error_message(err: &ParseError) -> &'static str {
        match err {
            ...
            ParseError::OnlyRegionRunesCanHaveMutability(_) => "Only region runes, such as 'x, can have ro or rw or mut.",
            ...
            ParseError::NeedWhitespaceAroundBinaryOperator(_) => "Need whitespace around binary operator.",
            ParseError::UnknownTupleOrSubExpression(_) => "Saw ( but expression is neither tuple nor sub-expression.",
            ...
            ParseError::UnrecognizedDenizenError(_) => "expected func, struct, interface, impl, import, or export, but found:",
            ParseError::BadFunctionBodyError(_) => "expected a function body, or `;` to note there is none. Found:",
            ...
            ParseError::IfBlocksMustBothOrNeitherReturn(_) => "If blocks should either both ret, or neither return.",
            ParseError::ForgotSetKeyword(_) => "Need `set` keyword to mutate a variable that already exists.",
            ...
            ParseError::BadLetSourceError { .. } => "Parse error somewhere inside this let source expression.",
            ...
            ParseError::FoundBothImmutableAndMutabilityInArray(_) => "Found both immutable and mutability in array.",
            ...
            ParseError::BadExternAttribute(_) => "Bad extern attribute. Expected string literal in parentheses.",
        }
    }
}
