...
//
// H-side output AST types for the simplifying (hammer) pass. Most of this
// file is the wrapped Scala source; the Rust definitions below are empty
// placeholders for the 15 types from ast.scala (ProgramH, PackageH, FunctionH,
// IdH, etc.). Fields are left empty (`pub struct FooH;` shape) — JR / a
// future migration pass will populate fields per the Scala when a fn body
// actually constructs one. For now these exist as named types so that
// signature references (`&IdH`, `Vec<FunctionH>`, etc.) across the
// simplifying-pass migration compile.
/*
...
/// Temporary state
#[derive(PartialEq, Eq, Hash, Clone)]
pub struct StructDefinitionH;

// mig: case class StructMemberH
/// Temporary state
#[derive(PartialEq, Eq, Hash, Clone)]
pub struct StructMemberH;

// mig: case class InterfaceDefinitionH
/// Temporary state
#[derive(PartialEq, Eq, Hash, Clone)]
pub struct InterfaceDefinitionH;

// mig: case class InterfaceMethodH
/// Temporary state
#[derive(PartialEq, Eq, Hash, Clone)]
pub struct InterfaceMethodH;
