...
1. `cargo build --manifest-path FrontendRust/Cargo.toml --lib` — clean, zero
   warnings (backend rebuilds via build.rs).
2. Full suite: `cargo nextest run --manifest-path FrontendRust/Cargo.toml`
   — expect the §3d numbers exactly.
3. Sweep greps — all must come back empty:

   ```
   grep -rn 'enableReplaying\|enable_replaying\|replay_whitelist\|RecordingMode\|vale_record\|vale_replay\|determinism' Backend/src FrontendRust/src
   grep -rn 'assert_replay_test\|replayReturnOrCallAndOrRecord\|replayExportCalls\|DeterministicMode' Backend/src FrontendRust/src
   ```
