...
- 8 direct `__vbi_` string-intrinsic tests
  ...
  `FrontendRust/src/end_to_end_tests/tests/strings.rs`. These lock in the
...
marked `#[ignore]` in `FrontendRust/src/end_to_end_tests/tests/replay.rs`.
...
`FrontendRust/src/builtins/resources/mainargs.vale` and its Scala mirror.
...
(from both `FrontendRust/src/builtins/resources/mainargs.vale` and the Scala
...
`FrontendRust/src/end_to_end_tests/mod.rs` contains a `KeepDir` enum with
...
- The `CompiledProgram::run` path sets `ASAN_OPTIONS=detect_leaks=1:
  ...
  when ASan is on. Suppressions file at `FrontendRust/lsan-suppressions.txt`
...
`FrontendRust/src/end_to_end_tests/mod.rs`. Under plain `cargo test` (without
...
Wired at `FrontendRust/src/end_to_end_tests/mod.rs:213-218`.
...
`FrontendRust/build.rs` watches the entire `Backend/src/` tree recursively
...
Slice 5b renamed string operators in both `FrontendRust/src/builtins/
