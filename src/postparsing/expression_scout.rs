...
use crate::parsing::ast::{
  ...
  LookupPE, NameP, SharednessP, StaticSizedArraySizeP,
};
...
use crate::postparsing::expressions::{
  BlockSE, ConstantBoolSE, ConstantIntSE, ConstantStrSE, CopyPrimSE, DestructSE, DotSE,
  ExprMutateSE, FunctionCallSE, FunctionSE, IExpressionSE, IfSE, IndexSE, LetSE, LoadPartSE,
  LocalLoadSE, LocalMutateSE, LocalS, NewRuntimeSizedArraySE, OutsideLoadSE, OverloadSetSE,
...
use crate::postparsing::post_parser::{
  ...
  InitializingStaticSizedArrayRequiresSizeAndCallable, PostParser, RangedInternalErrorS,
  StackFrame, StatementAfterReturnS, VariableNameAlreadyExists,
};
...
impl<'s, 'p, 'ctx> PostParser<'s, 'p, 'ctx> {
  ...
  pub(crate) fn scout_block(
    ...
    let context_region: IRuneS<'s> = match &block_pe.maybe_default_region {
      ...
      Some(region_rune_pt) => {
        ...
        let region_rune_s: IRuneS<'s> = self
          .scout_arena
          .intern_rune(IRuneValS::CodeRune(CodeRuneS::<'s> { name: region_rune_name_s }));
    ...
    let (block_s, self_uses_of_things_from_above, child_uses_of_things_from_above) = self
      .new_block(
        function_body_env,
        ...
        |stack_frame1, block_lidb| {
          let (stack_frame2, inner_expr_s, self_uses, child_uses) = self
            .scout_expression_and_coerce(stack_frame1, block_lidb, block_pe.inner, LoadAsP::Use)?;
    ...
    Ok((resulting_expr_s, self_uses_of_things_from_above, child_uses_of_things_from_above))
  }
  ...
  fn scout_impure_block(
    ...
    let (expr_s, self_uses_of_things_from_above, child_uses_of_things_from_above) =
      self.scout_block(parent_stack_frame, lidb, initial_locals, block_pe)?;
    match expr_s {
      IExpressionSE::Block(block_s) => {
        Ok((block_s, self_uses_of_things_from_above, child_uses_of_things_from_above))
      }
  ...
  where
    F: FnOnce(
      ...
      (StackFrame<'s>, &'s IExpressionSE<'s>, VariableUses<'s>, VariableUses<'s>),
      ICompileErrorS<'s>,
    >,
  {
    let maybe_parent = parent_stack_frame.clone().map(Box::new);
    let pure_height = parent_stack_frame.map(|parent| parent.pure_height + 1).unwrap_or(0);
    ...
    Ok((
      &*self.scout_arena.alloc(BlockSE::<'s> {
        range: range_s,
  ...
  fn scout_expression(
    ...
    (StackFrame<'s>, IScoutResult<'s, 'p>, VariableUses<'s>, VariableUses<'s>),
    ICompileErrorS<'s>,
  > {
    ...
    match expression {
      ...
      IExpressionPE::Return(ret) => {
        ...
        let (stack_frame1, inner_expr_s, inner_self_uses, inner_child_uses) = self
          .scout_expression_and_coerce(stack_frame, &mut ret_expr_lidb, ret.expr, LoadAsP::Use)?;
        Ok((
          stack_frame1,
          IScoutResult::NormalResult(NormalResultS {
      ...
      IExpressionPE::SubExpression(sub_expression) => {
        ...
        let (stack_frame1, sub_expression_s, sub_self_uses, sub_child_uses) = self
      ...
      IExpressionPE::Lookup(lookup) => match &lookup.template_args {
        None => {
          let range = PostParser::eval_range(&file_coordinate, lookup.name.range());
          let imprecise_name_s =
            translate_imprecise_name(self.scout_arena, &file_coordinate, &lookup.name);
          let lookup_result = match self.find_local(&stack_frame, range.clone(), &imprecise_name_s)
          {
            ...
            None => match &imprecise_name_s {
              IImpreciseNameS::CodeName(code_name) => {
                ...
                let code_rune =
                  self.scout_arena.intern_rune(IRuneValS::CodeRune(CodeRuneS { name }));
                if stack_frame.parent_env.all_declared_runes().contains(&code_rune) {
                  IScoutResult::NormalResult(NormalResultS {
                    expr: &*self
                      .scout_arena
                      .alloc(IExpressionSE::RuneLookup(RuneLookupSE { range, rune: code_rune })),
                  })
                } else {
        ...
        Some(template_args) => {
          let (range, template_name) = match &lookup.name {
            IImpreciseNameP::LookupName(name_p) => (
              PostParser::eval_range(&file_coordinate, name_p.range()),
      ...
      IExpressionPE::FunctionCall(function_call) => {
        ...
        match subject_uncoerced_scout_result {
          IScoutResult::OutsideLookupResult(OutsideLookupResultS {
            ...
            if container_name == "__copy_prim" {
              ...
              if function_call.arg_exprs.len() != 1 {
                panic!(
                  "__copy_prim expects exactly 1 argument, got {}",
                  function_call.arg_exprs.len()
                );
              }
              ...
              let (stack_frame_arg, arg_se, arg_self_uses, arg_child_uses) = self
                .scout_expression_and_coerce(
                  stack_frame0a,
                  &mut arg_lidb,
                  function_call.arg_exprs[0],
                  LoadAsP::LoadAsBorrow,
                )?;
              let result = IScoutResult::NormalResult(NormalResultS {
                expr: &*self
                  .scout_arena
                  .alloc(IExpressionSE::CopyPrim(CopyPrimSE { range, inner_expr: arg_se })),
              });
            ...
            let (stack_frame3, args_se, self_uses, child_uses) = self
              .scout_elements_as_expressions(
                stack_frame0a,
                &mut args_lidb,
                &function_call.arg_exprs,
              )?;
            let result = IScoutResult::NormalResult(NormalResultS {
          ...
          _ => {
            ...
            let mut args_lidb = lidb.child();
            let (stack_frame3, args1, args_self_uses, args_child_uses) = self
              .scout_elements_as_expressions(
                stack_frame2,
                &mut args_lidb,
                &function_call.arg_exprs,
      ...
      IExpressionPE::BinaryCall(binary_call) => {
        ...
        let result = IScoutResult::NormalResult(NormalResultS {
          expr: &*self.scout_arena.alloc(IExpressionSE::FunctionCall(FunctionCallSE {
            ...
            location: lidb.child().consume_in_arena(self.scout_arena),
            callable_expr: overload_set_se,
            arg_exprs: self.scout_arena.alloc_slice_from_vec(vec![left_expr_s, right_expr_s]),
          })),
        });
        Ok((
          stack_frame2,
      ...
      IExpressionPE::MethodCall(method_call) => {
        ...
        let (stack_frame3, args, self_uses, child_uses, container_parts): (
          StackFrame<'s>,
          ...
          IScoutResult::OutsideLookupResult(OutsideLookupResultS {
            ...
            let mut args_lidb = lidb.child();
            let (stack_frame3, args, self_uses, child_uses) = self.scout_elements_as_expressions(
              stack_frame0a,
              &mut args_lidb,
          ...
          _ => {
            ...
            let mut args_lidb = lidb.child();
            let (stack_frame3, tail_args1, tail_args_self_uses, tail_args_child_uses) = self
              .scout_elements_as_expressions(stack_frame1, &mut args_lidb, method_call.arg_exprs)?;
            let mut args = vec![subject1];
            args.extend(tail_args1);
            (
              stack_frame3,
              args,
              subject_self_uses.then_merge(&tail_args_self_uses),
              subject_child_uses.then_merge(&tail_args_child_uses),
              Vec::new(),
            )
          }
        };
        ...
        let result = match &construct_array.size {
          IArraySizeP::RuntimeSized => {
            ...
            if args_s.is_empty() || args_s.len() > 2 {
              return Err(ICompileErrorS::InitializingRuntimeSizedArrayRequiresSizeAndCallable(
          ...
          IArraySizeP::StaticSized(StaticSizedArraySizeP { size_pt: maybe_size_pt }) => {
            ...
            if construct_array.initializing_individual_elements {
              let size_rune_s = match maybe_size_rune_s {
                Some(s) => s,
                None => {
                  let rune = self.scout_arena.intern_rune(IRuneValS::ImplicitRune(
              ...
              if args_s.len() != 1 {
                return Err(ICompileErrorS::InitializingStaticSizedArrayRequiresSizeAndCallable(
      ...
      IExpressionPE::Block(block) => {
        ...
        let (result_se, self_uses, child_uses) = self.scout_block(
          stack_frame.clone(),
          &mut block_lidb,
      ...
      IExpressionPE::Consecutor(consecutor) => {
        ...
        let (stack_frame1, unfiltered_exprs, self_uses, child_uses) = self
          .scout_elements_as_expressions(stack_frame, &mut consecutor_lidb, &consecutor.inners)?;
        ...
        Ok((
          stack_frame1,
          IScoutResult::NormalResult(NormalResultS { expr: self.consecutive(filtered_exprs) }),
          self_uses,
          child_uses,
        ))
      }
      ...
      IExpressionPE::If(if_expr) => {
        ...
        let (result_se, self_uses, child_uses): (
          ...
          |stack_frame1: StackFrame<'s>, block_lidb: &mut LocationInDenizenBuilder| {
            ...
            Ok((stack_frame2, if_se, self_uses, child_uses))
          },
        )?;
        Ok((
          stack_frame,
      ...
      IExpressionPE::Let(lett) => {
        ...
        let declarations_from_pattern =
          VariableDeclarations { vars: get_parameter_captures(&pattern_s) };
        let maybe_name_conflict_var_name = stack_frame1
          .locals
          .vars
          .iter()
      ...
      IExpressionPE::Mutate(mutate) => {
        ...
        let (stack_frame2, destination_result_s, destination_self_uses, destination_child_uses): (
          ...
          self.scout_expression(stack_frame1, &mut destination_expr_lidb, mutate.mutatee)?
        };
        let (mutate_expr_s, source_self_uses): (&'s IExpressionSE<'s>, VariableUses<'s>) =
          match destination_result_s {
      ...
      IExpressionPE::Dot(dot) => {
        match dot.left {
          IExpressionPE::Lookup(LookupPE {
            name: IImpreciseNameP::LookupName(lookup_name),
            ..
        ...
        let (stack_frame1, container_expr_s, self_uses, child_uses) = {
          ...
          let (stack_frame1, container_expr_s, self_uses, child_uses) = self
            .scout_expression_and_coerce(
              stack_frame.clone(),
              &mut dot_left_lidb,
              dot.left,
      ...
      IExpressionPE::BraceCall(brace_call) => {
        ...
        let (stack_frame1, callable_se, callable_self_uses, callable_child_uses) = self
          .scout_expression_and_coerce(
            stack_frame,
            &mut lidb.child(),
            brace_call.subject_expr,
            load_subject_as,
          )?;
        let (stack_frame2, arg_se, arg_self_uses, arg_child_uses) = self
          .scout_expression_and_coerce(stack_frame1, &mut lidb.child(), arg_pe, LoadAsP::Use)?;
        ...
        Ok((
          stack_frame2,
          IScoutResult::NormalResult(NormalResultS { expr: self.scout_arena.alloc(result_se) }),
          callable_self_uses.then_merge(&arg_self_uses),
          callable_child_uses.then_merge(&arg_child_uses),
        ))
      }
      IExpressionPE::Destruct(destruct_pe) => {
        let (stack_frame1, inner1, inner_self_uses, inner_child_uses) = self
          .scout_expression_and_coerce(
            stack_frame,
            &mut lidb.child(),
            destruct_pe.inner,
      ...
      IExpressionPE::And(and_pe) => {
        ...
        let (stack_frame_z, if_se, self_uses, child_uses) = Self::new_if(
          ...
          |sf2, lidb| {
            let (then_se, then_uses, then_child_uses) = self.scout_impure_block(
              sf2.clone(),
              &mut lidb.child(),
              PostParser::<'s, 'p, '_>::no_declarations(),
              and_pe.right,
          ...
          |sf3, _lidb| {
            ...
            let else_se =
              self.scout_arena.alloc(BlockSE { range: end_range, locals: &[], expr: false_const });
        ...
        Ok((
          stack_frame_z,
          IScoutResult::NormalResult(NormalResultS { expr: if_alloc }),
          self_uses,
          child_uses,
        ))
      }
      IExpressionPE::Or(or_pe) => {
        ...
        let (stack_frame_z, if_se, self_uses, child_uses) = Self::new_if(
          ...
          |sf2, _lidb| {
            let true_const = self
              .scout_arena
              .alloc(IExpressionSE::ConstantBool(ConstantBoolSE { range: end_range, value: true }));
            let else_se =
              self.scout_arena.alloc(BlockSE { range: end_range, locals: &[], expr: true_const });
      ...
      IExpressionPE::StrInterpolate(str_interp_pe) => {
        let (stack_frame1, parts_se, parts_self_uses, parts_child_uses) = self
          .scout_elements_as_expressions(stack_frame, &mut lidb.child(), str_interp_pe.parts)?;
        ...
        Ok((
          stack_frame1,
          IScoutResult::NormalResult(NormalResultS { expr: added_expr }),
          parts_self_uses,
          parts_child_uses,
        ))
      }
      IExpressionPE::Break(break_pe) => {
        ...
        Ok((
          stack_frame,
          IScoutResult::NormalResult(NormalResultS { expr }),
          VariableUses::empty(),
          VariableUses::empty(),
        ))
      }
      IExpressionPE::Unlet(unlet_pe) => {
        ...
        let imprecise_name_s =
          translate_imprecise_name(self.scout_arena, &file_coordinate, &unlet_pe.name);
        ...
        Ok((
          ...
          VariableUses::empty().mark_moved(var_name_s),
          VariableUses::empty(),
        ))
      }
      IExpressionPE::Range(range_pe) => {
        ...
        let (stack_frame1, begin_se, begin_self_uses, begin_child_uses) = self
          .scout_expression_and_coerce(
            stack_frame,
            &mut lidb.child(),
            range_pe.from_expr,
        ...
        let (stack_frame2, end_se, end_self_uses, end_child_uses) = self
          .scout_expression_and_coerce(
            stack_frame1,
            &mut lidb.child(),
            range_pe.to_expr,
        ...
        Ok((
          stack_frame2,
          IScoutResult::NormalResult(NormalResultS { expr: result_se }),
          begin_self_uses.then_merge(&end_self_uses),
          begin_child_uses.then_merge(&end_child_uses),
        ))
      }
      _ => panic!("POSTPARSER_SCOUT_EXPRESSION_NOT_YET_IMPLEMENTED: {:?}", expression),
    }
  }
  ...
  where
    ...
    FThen: FnOnce(
      ...
      (StackFrame<'s>, &'s BlockSE<'s>, VariableUses<'s>, VariableUses<'s>),
      ICompileErrorS<'s>,
    >,
    FElse: FnOnce(
      StackFrame<'s>,
    ...
    >,
  {
    ...
    let (stack_frame1, cond_se, cond_uses, cond_child_uses): (
      StackFrame<'s>,
      &'s IExpressionSE<'s>,
      VariableUses<'s>,
      VariableUses<'s>,
    ) = make_condition(stack_frame0, &mut lidb.child())?;
    let (stack_frame2, then_se, then_uses, then_child_uses): (
      StackFrame<'s>,
      &'s BlockSE<'s>,
      VariableUses<'s>,
      VariableUses<'s>,
    ) = make_then(stack_frame1, &mut lidb.child())?;
    let (stack_frame3, else_se, else_uses, else_child_uses): (
      StackFrame<'s>,
      &'s BlockSE<'s>,
      VariableUses<'s>,
      VariableUses<'s>,
    ) = make_else(stack_frame2, &mut lidb.child())?;
  ...
  > {
    ...
    let (next_stack_frame, first_result_s, first_inner_self_uses, first_child_uses) =
      self.scout_expression(stack_frame, &mut expression_lidb, expression_p)?;
    let mut coerce_lidb = lidb.child();
    let (next_stack_frame, first_expr_s, first_self_uses) = self.coerce(
      next_stack_frame,
      first_result_s,
      &mut coerce_lidb,
  ...
  pub(crate) fn coerce(
    ...
    match first_result {
      ...
      IScoutResult::NormalResult(NormalResultS { expr: inner_expr_s }) => match load_as_p {
  ...
  pub(crate) fn scout_elements_as_expressions(
    ...
    (StackFrame<'s>, Vec<&'s IExpressionSE<'s>>, VariableUses<'s>, VariableUses<'s>),
    ICompileErrorS<'s>,
  > {
...
fn flatten_expressions<'s>(_expr: &IExpressionSE<'s>) -> Vec<&'s IExpressionSE<'s>> {
