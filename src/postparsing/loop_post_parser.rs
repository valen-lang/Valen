...
use crate::postparsing::expressions::{BlockSE, BreakSE, IExpressionSE, MapSE, VoidSE, WhileSE};
...
fn scout_loop<'s, F>(
  ...
  _pure: bool,
  _make_contents: F,
) -> (BlockSE<'s>, VariableUses<'s>, VariableUses<'s>)
where
  F: FnOnce(
    StackFrame<'s>,
    &mut LocationInDenizenBuilder,
    bool,
  ) -> (StackFrame<'s>, BlockSE<'s>, VariableUses<'s>, VariableUses<'s>),
{
  ...
  let (each_block_s, self_uses, child_uses) = post_parser.new_block(
    ...
    |stack_frame1, each_contents_lidb| {
      ...
      let (stack_frame3, let_iterator_se, let_iterator_self_uses, let_iterator_child_uses): (
        ...
        let begin_args: &'p [&'p IExpressionPE<'p>] =
          pa.alloc_slice_from_vec(vec![&*pa.alloc(iterable_borrow_expr_p)]);
        let begin_call_expr_p: &'p IExpressionPE<'p> =
          &*pa.alloc(IExpressionPE::FunctionCall(FunctionCallPE {
            range: in_keyword_range,
      ...
      let self_uses =
        let_iterable_self_uses.then_merge(&let_iterator_self_uses).then_merge(&loop_body_self_uses);
...
> {
  ...
  let (stack_frame4, if_se, if_self_uses, if_child_uses) = PostParser::new_if(
    ...
    |stack_frame1, condition_lidb| {
      ...
      let iterator_lookup_expr_p: &'p IExpressionPE<'p> =
        &*pa.alloc(IExpressionPE::Lookup(pa.alloc(LookupPE {
          name: IImpreciseNameP::IteratorName(in_keyword_range),
          template_args: None,
        })));
      let iterator_borrow_expr_p =
        IExpressionPE::Borrow(BorrowPE { range: in_keyword_range, inner: iterator_lookup_expr_p });
      let next_args: &'p [&'p IExpressionPE<'p>] =
        pa.alloc_slice_from_vec(vec![&*pa.alloc(iterator_borrow_expr_p)]);
      let next_call_expr_p: &'p IExpressionPE<'p> =
        &*pa.alloc(IExpressionPE::FunctionCall(FunctionCallPE {
          range: in_keyword_range,
          operator_range: in_keyword_range,
      ...
      let is_empty_lookup_expr_p: &'p IExpressionPE<'p> =
        &*pa.alloc(IExpressionPE::Lookup(pa.alloc(LookupPE {
      ...
      let is_empty_args: &'p [&'p IExpressionPE<'p>] =
        pa.alloc_slice_from_vec(vec![&*pa.alloc(iteration_option_borrow_expr_p)]);
      ...
      let condition_inners: &'p [&'p IExpressionPE<'p>] = pa.alloc_slice_from_vec(vec![
        &*pa.alloc(let_iteration_option_expr_p),
        &*pa.alloc(is_empty_call_expr_p),
      ]);
      let condition_expr_p: &'p IExpressionPE<'p> =
        &*pa.alloc(IExpressionPE::Consecutor(ConsecutorPE { inners: condition_inners }));
    ...
    |stack_frame1, then_lidb| {
      ...
      let (then_s, then_uses, then_child_uses) = post_parser.new_block(
        ...
        |stack_frame2, then_inner_lidb| {
          ...
          let break_s = &*post_parser
            .scout_arena
            .alloc(IExpressionSE::Break(BreakSE { range: each_range_s.clone() }));
    ...
    |stack_frame1, _else_lidb| {
      ...
      let else_s = &*post_parser.scout_arena.alloc(BlockSE {
        ...
        expr: &*post_parser
          .scout_arena
          .alloc(IExpressionSE::Void(VoidSE { range: each_range_s.clone() })),
      });
      Ok((
        stack_frame1,
        else_s,
  ...
  let (stack_frame5, consume_some_se, consume_some_self_uses, consume_some_child_uses): (
    ...
    let get_lookup_expr_p: &'p IExpressionPE<'p> =
      &*pa.alloc(IExpressionPE::Lookup(pa.alloc(LookupPE {
    ...
    let get_args: &'p [&'p IExpressionPE<'p>] =
      pa.alloc_slice_from_vec(vec![&*pa.alloc(iteration_option_lookup_expr_p)]);
    let get_call_expr_p: &'p IExpressionPE<'p> =
      &*pa.alloc(IExpressionPE::FunctionCall(FunctionCallPE {
        range: in_keyword_range,
        operator_range: in_keyword_range,
  ...
  let self_uses = if_self_uses.then_merge(&consume_some_self_uses).then_merge(&user_body_self_uses);
  let child_uses =
    if_child_uses.then_merge(&consume_some_child_uses).then_merge(&user_body_child_uses);
...
pub(crate) fn scout_while<'s, 'p, 'ctx>(
  ...
  let (loop_s, loop_body_self_uses, loop_body_child_uses) = post_parser.new_block(
    ...
    |stack_frame1, inner_lidb| {
        ...
        |stack_frame4, innermost_lidb| {
          ...
          let (loop_body_se, loop_body_self_uses, loop_body_child_uses) = post_parser.new_block(
            ...
            |stack_frame5, body_lidb| {
              scout_while_body(post_parser, stack_frame5, body_lidb, range, condition_pe, body)
            },
          )?;
      ...
      let inner_loop_expr = &*post_parser.scout_arena.alloc(IExpressionSE::Block(inner_loop_s));
      Ok((stack_frame1, inner_loop_expr, loop_body_self_uses, loop_body_child_uses))
    },
  )?;
...
> {
  ...
  let (stack_frame4, if_se, if_self_uses, if_child_uses) = PostParser::new_if(
    ...
    |stack_frame2, condition_lidb| {
      let (stack_frame3, cond_se, cond_self_uses, cond_child_uses) = post_parser
        .scout_expression_and_coerce(stack_frame2, condition_lidb, condition_pe, LoadAsP::Use)?;
      Ok((stack_frame3, cond_se, cond_self_uses, cond_child_uses))
    },
    |stack_frame2, _then_lidb| {
      ...
      let void_s = &*post_parser.scout_arena.alloc(BlockSE {
        ...
        expr: &*post_parser
          .scout_arena
          .alloc(IExpressionSE::Void(VoidSE { range: while_range_s.clone() })),
      });
      Ok((
        stack_frame2,
        void_s,
    ...
    |stack_frame3, else_lidb| {
      ...
      let (then_s, then_uses, then_child_uses) = post_parser.new_block(
        ...
        |stack_frame4, _break_lidb| {
          let break_s = &*post_parser
            .scout_arena
            .alloc(IExpressionSE::Break(BreakSE { range: while_range_s.clone() }));
          Ok((
            stack_frame4,
            break_s,
            PostParser::<'s, 'p, '_>::no_variable_uses(),
