...
use crate::postparsing::ast::{
  ...
  IGenericParameterTypeS, IStructMemberS, ImplS, ImportS, InterfaceS, KindGenericParameterTypeS,
  LocationInDenizenBuilder, MacroCallS, NormalStructMemberS, OtherGenericParameterTypeS, ProgramS,
  RegionGenericParameterTypeS, StructS, VariadicStructMemberS,
};
...
use crate::postparsing::rules::templex_scout::{
  add_literal_rule, translate_signature_type_st, translate_templex, translate_templex_into_type_st,
  ...
  pub fn all_declared_runes(&self) -> IndexSet<IRuneS<'s>> {
    match self {
      ...
      IEnvironmentS::FunctionEnvironment(function_environment) => {
        function_environment.all_declared_runes()
      }
    }
  }
...
impl<'s, 'p, 'ctx> PostParser<'s, 'p, 'ctx> {
  ...
  pub fn eval_range(file: &'s FileCoordinate<'s>, range: RangeL) -> RangeS<'s> {
    RangeS::new(Self::eval_pos(file, range.begin()), Self::eval_pos(file, range.end()))
  }

  pub fn eval_pos(file: &'s FileCoordinate<'s>, pos: i32) -> CodeLocationS<'s> {
    CodeLocationS { file, offset: pos }
  }
}
  ...
  pub(crate) fn scout_generic_parameter(
    ...
    let generic_param_type_s = match type_s {
      ...
      _ => {
        ...
        IGenericParameterTypeS::OtherGenericParameterType(OtherGenericParameterTypeS::new(type_s))
      }
    };
...
impl<'s, 'p, 'ctx> PostParser<'s, 'p, 'ctx> {
  ...
  pub fn new(
    ...
    Self { global_options, scout_arena, keywords, keywords_p, parse_arena }
  }
  ...
  pub fn scout_program(
    ...
    for denizen in parsed.denizens {
      if let IDenizenP::TopLevelInterface(interface_p) = denizen {
        interfaces
          .push(&*self.scout_arena.alloc(self.scout_interface(file_coordinate, interface_p)?));
      }
    }
  ...
  fn scout_impl(
    ...
    let template_rules_p: &[IRulexPR<'p>] =
      impl0.template_rules.as_ref().map(|template_rules_p| template_rules_p.rules).unwrap_or(&[]);
    ...
    let user_declared_runes_set =
      user_declared_runes.iter().map(|rune_usage| rune_usage.rune.clone()).collect::<Vec<_>>();
    ...
    let default_region_rune_range_s = RangeS::new(range_s.end.clone(), range_s.end.clone());
    ...
    let sub_citizen_type =
      translate_templex_into_type_st(self.scout_arena, impl_env.clone(), struct_);
    let struct_rune = {
      let mut child_lidb = lidb.child();
    ...
    let super_interface_type =
      translate_templex_into_type_st(self.scout_arena, impl_env.clone(), &impl0.interface);
    let interface_rune = {
      let mut child_lidb = lidb.child();
      let (full, _value, outer_vec, value_vec) = translate_signature_type_st(
        self.scout_arena,
        ...
        if matches!(call.template, ITemplexPT::NameOrRune(_))
          && match call.template {
            ITemplexPT::NameOrRune(name)
              if !user_declared_runes_set.contains(&self.scout_arena.intern_rune(
                IRuneValS::CodeRune(CodeRuneS {
  ...
  fn scout_export_as(
    ...
    let export_name =
      self.scout_arena.intern_name(INameValS::ExportAsName(ExportAsNameS { code_location: pos }));
    ...
    let export_tree =
      translate_templex_into_type_st(self.scout_arena, export_env.clone(), &export_as_p.struct_);
  ...
  fn scout_import(&self, file: &'s FileCoordinate<'s>, import_p: &ImportP<'p>) -> ImportS<'s> {
    ...
    ImportS {
      ...
      package_names: self.scout_arena.alloc_slice_from_vec(
        import_p
          .package_steps
          .iter()
          .map(|n| self.scout_arena.intern_str(n.str().as_str()))
          .collect(),
      ),
  ...
  fn scout_struct(
    ...
    let struct_name =
      self.scout_arena.intern_struct_declaration_name(TopLevelStructDeclarationNameS {
    ...
    let generic_parameters_p: &[GenericParameterP<'p>] =
      head.identifying_runes.as_ref().map(|x| x.params as &[GenericParameterP<'p>]).unwrap_or(&[]);
    let user_specified_identifying_runes = generic_parameters_p
      ...
      .collect::<Vec<_>>();
    let template_rules_p: &[IRulexPR<'p>] =
      head.template_rules.as_ref().map(|x| x.rules as &[IRulexPR<'p>]).unwrap_or(&[]);
    let runes_from_rules =
      get_ordered_rune_declarations_from_rulexes_with_duplicates(&template_rules_p)
        .iter()
        .map(|name_p| RuneUsage {
    ...
    let struct_env = IEnvironmentS::Environment(EnvironmentS {
      ...
      user_declared_runes: user_declared_runes.iter().map(|x| x.rune.clone()).collect(),
    });
    ...
    let (_default_region_rune_range_s, default_region_rune_s, _maybe_region_generic_param) =
      match &head.maybe_default_region_rune {
        None => {
          let region_range = RangeS::new(body_range_s.begin.clone(), body_range_s.begin.clone());
          let rune = self.scout_arena.intern_rune(IRuneValS::DenizenDefaultRegionRune(
            DenizenDefaultRegionRuneS {
              denizen_name: self
                .scout_arena
                .intern_name(INameValS::TopLevelStructDeclaration(struct_name.clone())),
            },
          ));
          let implicit_region_generic_param = GenericParameterS {
            range: region_range.clone(),
            rune: RuneUsage { range: region_range.clone(), rune: rune.clone() },
    ...
    let struct_user_specified_generic_parameters_s: Vec<&'s GenericParameterS<'s>> =
      generic_parameters_p
        .iter()
        .zip(user_specified_identifying_runes.iter())
        .map(|(g, r)| {
    ...
    let all_rules_s =
      header_rules_s.iter().chain(member_rules_s.iter()).cloned().collect::<Vec<_>>();

    let param_types_vec: Vec<ITemplataType<'s>> =
      generic_parameters_s.iter().map(|x| x.tyype.tyype()).collect();
    ...
    let generic_parameters_s_arena: &'s [&'s GenericParameterS<'s>] =
      self.scout_arena.alloc_slice_from_vec(generic_parameters_s.clone());
    let internal_methods_s_vec: Vec<&'s FunctionS<'s>> = internal_methods_p
      .iter()
      .map(|method| -> Result<&'s FunctionS<'s>, ICompileErrorS<'s>> {
    ...
    attrs_p
      ...
      .map(|attr_p| match attr_p {
        IAttributeP::ExportAttribute(_) => {
          ICitizenAttributeS::Export(ExportS { package_coordinate: file.package_coord })
        }
  ...
  fn scout_interface(
    ...
    let interface_name =
      self.scout_arena.intern_interface_declaration_name(TopLevelInterfaceDeclarationNameS {
        name: self.scout_arena.intern_str(interface.name.str().as_str()),
        range: Self::eval_range(file, interface.name.range()),
      });
    let rules_p: &[IRulexPR<'p>] =
      interface.template_rules.as_ref().map(|x| x.rules as &[IRulexPR<'p>]).unwrap_or(&[]);
    ...
    let runes_from_rules = get_ordered_rune_declarations_from_rulexes_with_duplicates(&rules_p)
      .iter()
      .map(|name_p| RuneUsage {
        range: Self::eval_range(file, name_p.range()),
        rune: self.scout_arena.intern_rune(IRuneValS::CodeRune(CodeRuneS {
    ...
    let interface_env = IEnvironmentS::Environment(EnvironmentS {
      ...
      name: self
        .scout_arena
        .intern_name(INameValS::TopLevelInterfaceDeclaration(interface_name.clone())),
      user_declared_runes,
    });
    ...
    let generic_parameters_s: &'s [&'s GenericParameterS<'s>] =
      self.scout_arena.alloc_slice_from_vec(generic_parameters_s);
  ...
  impl<'s, 'ctx, 'p> ScoutCompilation<'s, 'ctx, 'p> {
    ...
    pub fn get_scoutput(
      ...
      for (file_coordinate_p, (file_p, _comments_and_ranges)) in &parseds.file_coord_to_contents {
        let package_coord_s: &'s PackageCoordinate<'s> =
          self.scout_arena.intern_package_coordinate(
        ...
        let file_coordinate_s: &'s FileCoordinate<'s> = self
          .scout_arena
          .intern_file_coordinate(package_coord_s, file_coordinate_p.filepath.as_str());
      ...
      for (pkg_p, files_p) in &parseds.package_coord_to_file_coords {
        ...
        let files_s: Vec<&'s FileCoordinate<'s>> = files_p
          .iter()
          .map(|fc| self.scout_arena.intern_file_coordinate(pkg_s, fc.filepath.as_str()))
          .collect();
