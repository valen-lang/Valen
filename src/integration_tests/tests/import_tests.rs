...
".to_string();
    ...
    let code_source: &CodeSource = compilation_bump.alloc(CodeSource::new(vec![
...
".to_string();
    ...
    let typing_interner = TypingInterner::new(&typing_bump);
    let module_a_coord = parse_arena.intern_package_coordinate(parse_arena.intern_str("moduleA"), &[]);
    let module_b_bork_coord = parse_arena.intern_package_coordinate(
        parse_arena.intern_str("moduleB"),
        &[parse_arena.intern_str("bork")]);
    let mut map: FileCoordinateMap<String> = FileCoordinateMap::new();
    map.put(
        parse_arena.intern_file_coordinate(module_a_coord, "moduleA.vale"),
        module_a_code);
