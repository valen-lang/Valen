...
use crate::typing::hinputs_t::{
  InstantiationBoundArgumentsT, InstantiationReachableBoundArgumentsT,
};
...
pub fn replace_value_type_in_ref<'s, 't>(
  ...
  match full_type_maybe_with_refs {
    ...
    KindT::OwnRef(o) => KindT::OwnRef(
      interner
        .alloc(OwnRefT { inner: replace_value_type_in_ref(interner, o.inner, new_value_type) }),
    ),
    KindT::ShareRef(s) => KindT::ShareRef(
      interner
        .alloc(ShareRefT { inner: replace_value_type_in_ref(interner, s.inner, new_value_type) }),
    ),
    KindT::WeakRef(w) => KindT::WeakRef(
      interner
        .alloc(WeakRefT { inner: replace_value_type_in_ref(interner, w.inner, new_value_type) }),
    ),
    _ => new_value_type,
  }
}
...
where
  's: 't,
{
  pub fn get_top_level_denizen_id(&self, id: IdT<'s, 't>) -> IdT<'s, 't> {
    let steps = id.steps();
    let is_instantiation_name =
      |name: &INameT<'s, 't>| -> bool { IInstantiationNameT::try_from(*name).is_ok() };
    ...
    assert!(
      is_instantiation_name(&last_step),
      "get_top_level_denizen_id: step at index is not IInstantiationNameT"
    );
  ...
  pub fn get_placeholder_templata_id(impl_placeholder: ITemplataT<'s, 't>) -> IdT<'s, 't> {
    ...
    let local_name = match id.local_name {
      INameT::Struct(s) => match s.template {
        ...
        IStructTemplateNameT::AnonymousSubstructTemplate(tmpl) => {
          INameT::AnonymousSubstructTemplate(tmpl)
        }
      },
  ...
  pub fn get_name_template(name: INameT<'s, 't>) -> INameT<'s, 't> {
  ...
  pub fn get_template(interner: &TypingInterner<'s, 't>, id: IdT<'s, 't>) -> &'t IdT<'s, 't> {
  ...
  pub fn get_sub_kind_template(&self, id: IdT<'s, 't>) -> IdT<'s, 't> {
    let last = IInstantiationNameT::try_from(id.local_name).unwrap_or_else(|_| {
      panic!("get_sub_kind_template: unexpected local_name {:?}", id.local_name)
    });
  ...
  pub fn get_super_kind_template(&self, id: IdT<'s, 't>) -> IdT<'s, 't> {
    let last = ISuperKindNameT::try_from(id.local_name).unwrap_or_else(|_| {
      panic!("get_super_kind_template: unexpected local_name {:?}", id.local_name)
    });
  ...
  pub fn get_struct_template(&self, id: IdT<'s, 't>) -> IdT<'s, 't> {
    let local_name = match id.local_name {
      INameT::Struct(s) => match s.template {
        IStructTemplateNameT::StructTemplate(tmpl) => INameT::StructTemplate(tmpl),
        IStructTemplateNameT::LambdaCitizenTemplate(tmpl) => INameT::LambdaCitizenTemplate(tmpl),
  ...
  pub fn get_impl_template(interner: &TypingInterner<'s, 't>, id: IdT<'s, 't>) -> IdT<'s, 't> {
    ...
    *interner.intern_id(crate::typing::names::names::IdValT {
      package_coord,
      init_steps,
      local_name: template,
    })
    ...
    match kind {
      ...
      KindT::RuntimeSizedArray(rsa) => {
        ...
        let new_arr_name = interner.intern_raw_array_name(RawArrayNameT {
          element_type: Self::substitute_templatas_in_kind(
            coutputs,
            sanity_check,
            interner,
            keywords,
        ...
        let new_rsa =
          interner.intern_runtime_sized_array_tt(RuntimeSizedArrayTTValT { name: new_id });
      ...
      KindT::StaticSizedArray(ssa) => {
        ...
        let new_ssa =
          interner.intern_static_sized_array_tt(StaticSizedArrayTTValT { name: new_id });
      ...
      KindT::Struct(s) => {
        let new_struct = Compiler::substitute_templatas_in_struct(
          coutputs,
          sanity_check,
          interner,
          keywords,
  ...
  pub fn substitute_templatas_in_struct(
    ...
    let new_local_name = match id.local_name {
      INameT::AnonymousSubstruct(asub_name_t) => {
        let new_template_args: Vec<ITemplataT<'s, 't>> = asub_name_t
          .template_args
          .iter()
          .map(|templata| {
            Self::substitute_templatas_in_templata(
              coutputs,
              sanity_check,
              interner,
              keywords,
      ...
      INameT::Struct(struct_name_t) => {
        let new_template_args: Vec<ITemplataT<'s, 't>> = struct_name_t
          .template_args
          .iter()
          .map(|templata| {
            Self::substitute_templatas_in_templata(
      ...
      INameT::LambdaCitizen(lambda_citizen_name_t) => INameT::LambdaCitizen(lambda_citizen_name_t),
  ...
  pub fn translate_instantiation_bounds(
    ...
    match bound_arguments_source {
      ...
      IBoundArgumentsSource::UseBoundsFromContainer {
        instantiation_bound_params: container_instantiation_bound_params,
        instantiation_bound_arguments: container_instantiation_bound_args,
      } => {
        let container_func_bound_to_bound_arg: HashMap<PrototypeT<'s, 't>, PrototypeT<'s, 't>> =
          container_instantiation_bound_args
            .rune_to_bound_prototype
            .iter()
            .map(|(rune, container_func_bound_arg)| {
              let param_proto =
                *container_instantiation_bound_params.rune_to_bound_prototype.get(rune).unwrap();
        ...
        let container_impl_bound_to_bound_arg: HashMap<IdT<'s, 't>, IdT<'s, 't>> =
          container_instantiation_bound_args
            ...
            .map(|(rune, container_impl_bound_arg)| {
              let param_impl =
                *container_instantiation_bound_params.rune_to_bound_impl.get(rune).unwrap();
          ...
          instantiation_bound_args.rune_to_bound_prototype.iter().map(|(rune, func_bound_arg)| {
            let new_val = match func_bound_arg.id.local_name {
              ...
              _ => {
                ...
                *Self::substitute_templatas_in_prototype(
                  coutputs,
                  sanity_check,
                  interner,
                  keywords,
        ...
        let rune_to_citizen_rune_to_reachable_prototype = interner.alloc_index_map_from_iter(
          instantiation_bound_args.rune_to_citizen_rune_to_reachable_prototype.iter().map(
            |(callee_rune, reachable_bound_args)| {
              let new_citizen = interner.alloc_index_map_from_iter(
                reachable_bound_args.citizen_rune_to_reachable_prototype.iter().map(
                  |(citizen_rune, reachable_prototype)| {
              ...
              let new_reachable: &'t InstantiationReachableBoundArgumentsT<'s, 't> = interner
                .alloc(InstantiationReachableBoundArgumentsT {
                  citizen_rune_to_reachable_prototype: new_citizen,
                });
        ...
        let rune_to_bound_impl = interner.alloc_index_map_from_iter(
          instantiation_bound_args.rune_to_bound_impl.iter().map(|(rune, impl_bound_arg)| {
            let new_val = match impl_bound_arg.local_name {
              ...
              _ => {
                // Not sure if this call is really necessary...
                Self::substitute_templatas_in_impl_id(
                  coutputs,
                  sanity_check,
                  interner,
                  keywords,
  ...
  pub fn substitute_templatas_in_bounds(
    ...
    let rune_to_citizen_rune_to_reachable_prototype = interner.alloc_index_map_from_iter(
      bound_args.rune_to_citizen_rune_to_reachable_prototype.iter().map(
        |(caller_rune, reachable_bound_args)| {
          ...
          let new_reachable: &'t InstantiationReachableBoundArgumentsT<'s, 't> =
            interner.alloc(InstantiationReachableBoundArgumentsT {
              citizen_rune_to_reachable_prototype: new_citizen_rune_to_reachable_prototype,
            });
  ...
  pub fn substitute_templatas_in_prototype(
    ...
    let substituted_template_args_vec: Vec<ITemplataT<'s, 't>> = func_name
      .template_args()
      .iter()
      .map(|templata| {
        Self::substitute_templatas_in_templata(
    ...
    let substituted_params_vec: Vec<KindT<'s, 't>> = func_name
      .parameters()
      .iter()
      .map(|coord| {
        Self::substitute_templatas_in_kind(
          coutputs,
          sanity_check,
          interner,
          keywords,
    ...
    let substituted_return_type = Self::substitute_templatas_in_kind(
      coutputs,
      sanity_check,
      interner,
      keywords,
      ...
      new_substituting_templatas,
      bound_arguments_source,
      original_prototype.return_type,
    );
    let substituted_func_name = func_name.template().make_function_name(
      interner,
      keywords,
  ...
  's: 't,
{
  pub fn get_placeholder_substituter(
    ...
    let top_level_denizen_template_id =
      Compiler::get_template(self.typing_interner, top_level_denizen_id);
  ...
  pub fn evaluate_templex(
    ...
    match templex {
      ...
      ITypeST::DynInterface(b) => {
        ...
        match self.evaluate_templex(env, rune_to_kind, b.inner).expect_kind() {
          KindT::Interface(iface) =>
            ITemplataT::Kind(KindTemplataT {
              kind: KindT::DynInterface(
                self.typing_interner.intern_dyn_interface_tt(DynInterfaceTTValT { inner: iface }),
              ),
            }),
  ...
  pub fn get_reachable_bounds(
    ...
    let (foreign_generic_runes, foreign_citizen_header_rules) = match citizen {
      ICitizenTT::Struct(_) => {
        ...
        let runes: Vec<_> =
          struct_s.generic_params.iter().map(|generic_param| generic_param.rune.rune).collect();
        (runes, struct_s.header_rules)
      }
      ICitizenTT::Interface(_) => {
        let interface_s = coutputs.get_postparsed_interface(&citizen_template_id);
        let runes: Vec<_> =
          interface_s.generic_params.iter().map(|generic_param| generic_param.rune.rune).collect();
  ...
  pub fn get_first_unsolved_identifying_rune(
    ...
    generic_parameters
      .iter()
      .enumerate()
      .map(|(index, generic_param)| {
        (generic_param, index as i32, is_solved(generic_param.rune.rune))
      })
...
impl<'a, 's, 't> IRuneTypeSolverEnv<'s, 't> for TemplataCompilerRuneTypeSolverEnv<'a, 's, 't>
where
  's: 't,
{
  fn lookup(
    ...
    match name_s {
      ...
      IImpreciseNameS::LambdaStructImpreciseName(_) => {
        Ok(IRuneTypeSolverLookupResult::Templata(TemplataLookupResult {
          templata: ITemplataType::KindTemplataType(KindTemplataType {}),
        }))
      }
      _ => {
  ...
  's: 't,
{
  ...
  pub fn is_type_convertible(
    ...
    match (&source_type, &target_type) {
      ...
      (a, b) if a == b => {}
      (KindT::DynInterface(a), KindT::Interface(b)) if a.inner == *b => {}
      (
        KindT::Void(_)
        | KindT::Int(_)
        | KindT::Bool(_)
  ...
  pub fn create_non_kind_non_region_placeholder_inner(
    ...
    let id_t = name_prefix
      .add_step(self.typing_interner, INameT::NonKindNonRegionPlaceholder(placeholder_name));
    // PlaceholderTemplataT(idT, tyype)
    ITemplataT::Placeholder(self.typing_interner.alloc(PlaceholderTemplataT { id: *id_t, tyype }))
  }
}
