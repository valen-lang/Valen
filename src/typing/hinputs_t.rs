...
pub fn make<'s, 't>(
  ...
  rune_to_citizen_rune_to_reachable_prototype: Vec<(
    IRuneS<'s>,
  ...
  interner.alloc(InstantiationBoundArgumentsT {
    rune_to_bound_prototype: interner
      .alloc_index_map_from_iter(rune_to_bound_prototype.into_iter()),
    rune_to_citizen_rune_to_reachable_prototype: interner
      .alloc_index_map_from_iter(rune_to_citizen_rune_to_reachable_prototype.into_iter()),
...
pub struct InstantiationBoundArgumentsT<'s, 't> {
  ...
  pub rune_to_citizen_rune_to_reachable_prototype:
    ArenaIndexMap<'t, IRuneS<'s>, &'t InstantiationReachableBoundArgumentsT<'s, 't>>,
...
pub struct HinputsT<'s, 't> {
  ...
  pub sub_citizen_to_interface_to_edge:
    HashMap<IdT<'s, 't>, HashMap<IdT<'s, 't>, &'t EdgeT<'s, 't>>>,
}
...
impl<'s, 't> HinputsT<'s, 't> {
  ...
  pub fn lookup_struct_by_template(
    &self,
    struct_template_name: StructTemplateNameT,
  ) -> StructDefinitionT<'s, 't> {
  ...
  pub fn lookup_interface_by_template(
    &self,
    interface_template_name: InterfaceTemplateNameT,
  ) -> InterfaceDefinitionT<'s, 't> {
  ...
  pub fn lookup_edge(&self, impl_id: IdT<'s, 't>) -> &'t EdgeT<'s, 't> {
    ...
    assert!(
      matches.len() == 1,
      "vassertOne: expected exactly one edge for impl_id {:?}, got {}",
      impl_id,
      matches.len()
    );
    *matches[0]
  ...
  pub fn lookup_citizen_by_template_id(
    &self,
    citizen_template_id: IdT<'s, 't>,
  ...
  pub fn lookup_struct_by_template_name(
    &self,
    struct_template_name: StructTemplateNameT<'s>,
  ) -> &'t StructDefinitionT<'s, 't> {
    let matches: Vec<&'t StructDefinitionT<'s, 't>> = self
      .structs
      .iter()
  ...
  pub fn lookup_interface_by_template_name(
    &self,
    interface_template_name: &'t InterfaceTemplateNameT<'s>,
  ...
  pub fn lookup_function_by_signature(
    &self,
    signature2: SignatureT<'s, 't>,
  ...
  pub fn lookup_function_by_template(
    &self,
    func_template_name: FunctionTemplateNameT,
  ) -> Option<&'t FunctionDefinitionT<'s, 't>> {
  ...
  pub fn lookup_function_by_str(&self, human_name: &str) -> &'t FunctionDefinitionT<'s, 't> {
    let matches: Vec<_> = self
      .functions
      .iter()
      .filter(|f| match &f.header.id.local_name {
  ...
  pub fn lookup_interface_by_human_name(
    &self,
    human_name: &str,
  ) -> &'t InterfaceDefinitionT<'s, 't> {
    let matches: Vec<_> = self
      .interfaces
      .iter()
      .filter(|i| match &i.template_name.local_name {
  ...
  pub fn name_is_lambda_in(&self, name: IdT<'s, 't>, needle_function_human_name: &str) -> bool {
    ...
    match (first, last_two) {
      (INameT::Function(f), [INameT::LambdaCitizenTemplate(_), INameT::LambdaCallFunction(_)])
        if f.template.human_name.0 == needle_function_human_name =>
      {
        true
      }
      _ => false,
  ...
  pub fn lookup_lambdas_in(
    &self,
    needle_function_human_name: &str,
  ) -> Vec<&'t FunctionDefinitionT<'s, 't>> {
    self
      .functions
      .iter()
      .copied()
      .filter(|f| self.name_is_lambda_in(f.header.id, needle_function_human_name))
      .collect()
  }

  pub fn lookup_lambda_in(
    &self,
    needle_function_human_name: &str,
  ) -> &'t FunctionDefinitionT<'s, 't> {
