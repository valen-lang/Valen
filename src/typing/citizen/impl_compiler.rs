...
use crate::typing::types::types::{InterfaceTT, KindT};
  ...
  's: 't,
{
  pub fn resolve_impl(
    ...
    let impl_a = coutputs.get_postparsed_impl(impl_templata.impl_template_id);

    let impl_template_name: IImplTemplateNameT<'s, 't> = self.translate_impl_name(
      impl_a.name,
    ...
    let impl_template_name_local: INameT<'s, 't> = match impl_template_name {
      ...
      IImplTemplateNameT::AnonymousSubstructImplTemplate(r) => {
        INameT::AnonymousSubstructImplTemplate(r)
      }
    };
    let impl_template_id: &'t IdT<'s, 't> =
      parent_env.id().add_step(self.typing_interner, impl_template_name_local);

    let outer_env_store = {
      let store = TemplatasStoreBuilder::new(impl_template_id);
      store.build_in(self.typing_interner)
    };
    let outer_env: &'t CitizenEnvironmentT<'s, 't> =
      self.typing_interner.alloc(CitizenEnvironmentT {
    ...
    impl_self_runes
      .insert(impl_a.struct_kind_rune.rune, ITemplataType::KindTemplataType(KindTemplataType {}));
    impl_self_runes.insert(
      impl_a.interface_kind_rune.rune,
  ...
  pub fn compile_impl(
    ...
    let impl_outer_env: &'t CitizenEnvironmentT<'s, 't> =
      self.typing_interner.alloc(CitizenEnvironmentT {
        global_env: parent_env.global_env(),
        parent_env: IEnvironmentT::from(parent_env),
        template_id: *impl_template_id,
    ...
    let reachable_bounds_from_sub_citizen =
      &complete_define_solve.rune_to_bound.rune_to_citizen_rune_to_reachable_prototype;
    ...
    let super_interface: &'t InterfaceTT<'s, 't> =
    ...
    let template_args: Vec<ITemplataT<'s, 't>> = impl_a
      .user_specified_identifying_runes
      .iter()
      .map(|p| *inferences.get(&p.rune.rune).expect("rune in inferences"))
      .collect();
    let instantiated_id: IdT<'s, 't> =
      self.assemble_impl_name(*impl_template_id, &template_args, sub_citizen);
    ...
    let mut inner_env_entries: Vec<(INameT<'s, 't>, IEnvEntryT<'s, 't>)> =
      reachable_bounds_from_sub_citizen
        .iter()
    ...
    let rune_to_needed_function_bound =
      self.assemble_rune_to_function_bound(impl_inner_env.templatas);
    let rune_to_needed_impl_bound = self.assemble_rune_to_impl_bound(impl_inner_env.templatas);

    let rune_index_to_independence = self.calculate_runes_independence(
      coutputs,
      call_location,
      impl_templata,
      impl_outer_env_iden,
    ...
    let mut rune_to_reachable: ArenaIndexMap<
      't,
      IRuneS<'s>,
    ...
    let instantiation_bound_params = self.typing_interner.alloc(InstantiationBoundArgumentsT {
      rune_to_bound_prototype: self
        .typing_interner
        .alloc_index_map_from_iter(rune_to_needed_function_bound.into_iter().map(|(k, v)| (k, *v))),
      rune_to_citizen_rune_to_reachable_prototype: rune_to_reachable,
      rune_to_bound_impl: self
        .typing_interner
        .alloc_index_map_from_iter(rune_to_needed_impl_bound.into_iter()),
    });
    ...
    let impl_t = ImplT {
      ...
      rune_index_to_independence: self
        .typing_interner
        .alloc_slice_from_vec(rune_index_to_independence),
    };
  ...
  pub fn assemble_impl_name(
    ...
    let new_local_name =
      impl_template_name.make_impl_name(self.typing_interner, template_args, sub_citizen);
  ...
  pub fn get_impl_parent_given_sub_citizen(
    ...
    let parent_tt = conclusions
      .get(&impl_a.interface_kind_rune.rune)
  ...
  pub fn get_parents(
    ...
    let impl_imprecise_name =
      self.scout_arena.intern_imprecise_name(IImpreciseNameValS::ImplSubCitizenImpreciseName(
        ImplSubCitizenImpreciseNameValS { sub_citizen_imprecise_name: sub_kind_imprecise_name },
      ));
    ...
    matching.extend(sub_kind_env.lookup_all_with_imprecise_name(
      impl_imprecise_name,
      lookup_filter.clone(),
      self.typing_interner,
    ));
    matching.extend(calling_env.lookup_all_with_imprecise_name(
      impl_imprecise_name,
      lookup_filter,
      self.typing_interner,
    ));
    ...
    let impl_defs: Vec<ImplDefinitionTemplataT<'s, 't>> = impl_defs_with_duplicates
      .into_iter()
      .filter(|d| seen_ranges.insert(coutputs.get_postparsed_impl(d.impl_template_id).range))
      .collect();
    let parents_from_impl_defs: Vec<ISuperKindTT<'s, 't>> = impl_defs
      .iter()
      .flat_map(|impl_def| {
        match ICitizenTT::try_from(sub_kind) {
          Ok(sub_citizen) => {
            match self.get_impl_parent_given_sub_citizen(
              coutputs,
              parent_ranges,
              call_location,
              calling_env,
  ...
  pub fn is_parent(
    ...
    let super_kind_imprecise_name =
      match get_imprecise_name(self.scout_arena, super_kind_tt.id().local_name) {
        None => return IsParentResult::IsntParent(IsntParent { candidates: vec![] }),
        Some(n) => n,
      };
    let sub_kind_imprecise_name =
      match get_imprecise_name(self.scout_arena, sub_kind_tt.id().local_name) {
        None => return IsParentResult::IsntParent(IsntParent { candidates: vec![] }),
        Some(n) => n,
      };
    let impl_imprecise_name = self.scout_arena.intern_imprecise_name(
      IImpreciseNameValS::ImplImpreciseName(ImplImpreciseNameValS {
        sub_citizen_imprecise_name: sub_kind_imprecise_name,
        super_interface_imprecise_name: super_kind_imprecise_name,
      }),
    );
    ...
    matching.extend(super_kind_env.lookup_all_with_imprecise_name(
      impl_imprecise_name,
      lookup_filter,
      self.typing_interner,
    ));
    ...
    let results: Vec<
      Result<
        (ImplDefinitionTemplataT<'s, 't>, CompleteResolveSolve<'s, 't>),
        IResolvingError<'s, 't>,
      >,
    > = impl_defs
      .iter()
      .map(|impl_def| {
        let impl_a = coutputs.get_postparsed_impl(impl_def.impl_template_id);
    ...
    match oks.into_iter().next() {
      Some(Ok((impl_templata, CompleteResolveSolve { conclusions, rune_to_bound }))) => {
        ...
        let template_args: Vec<ITemplataT<'s, 't>> = impl_a
          .user_specified_identifying_runes
          .iter()
          .map(|p| *conclusions.get(&p.rune.rune).unwrap())
          .collect();
        let impl_template_name: INameT<'s, 't> = match self.translate_impl_name(
          impl_a.name,
          impl_a.sub_citizen_imprecise_name,
          impl_a.super_interface_imprecise_name,
        ) {
        ...
        let impl_template_id =
          impl_templata.env.id().add_step(self.typing_interner, impl_template_name);
        let instantiated_id =
          self.assemble_impl_name(*impl_template_id, &template_args, sub_kind_tt.expect_citizen());
        coutputs.add_instantiation_bounds(
          self.opts.global_options.sanity_check,
          self.typing_interner,
      ...
      None => {
        let err_vec: Vec<IResolvingError<'s, 't>> = errs
          .into_iter()
          .map(|r| match r {
            Err(e) => e,
            Ok(_) => unreachable!(),
