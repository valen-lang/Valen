  ...
  's: 't,
{
  pub fn resolve_struct_layer(
    ...
    let initial_knowns: Vec<InitialKnown<'s, 't>> = struct_a
      .generic_params
      .iter()
      .zip(template_args.iter())
      .map(|(generic_param, template_arg)| InitialKnown {
        rune: generic_param.rune,
        templata: *template_arg,
      })
      .collect();
    ...
    let header_rune_to_type_map: IndexMap<IRuneS<'s>, ITemplataType<'s>> = self
      .derive_rune_to_type(
        coutputs,
        original_calling_env,
        call_range.to_vec(),
    ...
    let complete_resolve_solve = match self
      ...
      Ok(ccs) => ccs,
      Err(x) => {
        return IResolveOutcome::ResolveFailure(ResolveFailure {
    ...
    let final_generic_args: Vec<ITemplataT<'s, 't>> = struct_a
      .generic_params
      .iter()
      .map(|gp| *complete_resolve_solve.conclusions.get(&gp.rune.rune).unwrap())
      .collect();
    let struct_name =
      struct_template_name.make_struct_name(self.typing_interner, &final_generic_args);
  ...
  pub fn predict_interface_layer(
    ...
    let InterfaceDefinitionTemplataT { declaring_env, interface_template_id: interface_id, .. } =
      interface_templata;
    ...
    let initial_knowns: Vec<InitialKnown<'s, 't>> = interface_a
      .generic_params
      .iter()
      .zip(template_args.iter())
      .map(|(generic_param, template_arg)| InitialKnown {
        rune: RuneUsage {
          range: *call_range.first().expect("vassertSome: callRange.headOption"),
          rune: generic_param.rune.rune,
        },
        templata: *template_arg,
      })
      .collect();
    ...
    let runes_for_prediction: HashSet<IRuneS<'s>> = interface_a
      ...
      .collect();
    let defaults_rune_to_type: IndexMap<IRuneS<'s>, ITemplataType<'s>> = interface_a
      .generic_params
      .iter()
    ...
    let rune_to_type_for_prediction: IndexMap<IRuneS<'s>, ITemplataType<'s>> = runes_for_prediction
      ...
      .map(|r| {
        (
          *r,
          interface_rune_to_type.get(r).copied().unwrap_or_else(|| {
            *defaults_rune_to_type.get(r).expect("rune not in runeToType or defaultsRuneToType")
          }),
        )
      })
      .collect();
    ...
    let inferences = match self.partial_solve(
      InferEnv {
        original_calling_env,
        parent_ranges: call_range,
        call_location,
    ...
    let final_generic_args: Vec<ITemplataT<'s, 't>> = interface_a
      .generic_params
      .iter()
      .map(|gp| *inferences.get(&gp.rune.rune).expect("rune not in inferences"))
      .collect();
    let interface_name =
      interface_template_name.make_interface_name(self.typing_interner, &final_generic_args);
  ...
  pub fn predict_struct_layer(
    ...
    let StructDefinitionTemplataT { declaring_env, struct_template_id: struct_id, .. } =
      struct_templata;
    ...
    let runes_for_prediction: HashSet<IRuneS<'s>> = struct_a
      ...
      .chain(call_site_rule_runes.into_iter())
      .collect();
    let defaults_rune_to_type: IndexMap<IRuneS<'s>, ITemplataType<'s>> = struct_a
      .generic_params
      .iter()
      .filter_map(|gp| gp.default.as_ref().map(|d| (d.result_rune, gp.tyype.tyype())))
      .collect();
    ...
    let rune_to_type_for_prediction: IndexMap<IRuneS<'s>, ITemplataType<'s>> = runes_for_prediction
      ...
      .map(|r| {
        (
          *r,
          header_rune_to_type.get(r).copied().unwrap_or_else(|| {
            *defaults_rune_to_type
              .get(r)
              .expect("rune not in headerRuneToType or defaultsRuneToType")
          }),
        )
      })
      .collect();
  ...
  pub fn resolve_interface_layer(
    ...
    let rune_to_type_map: IndexMap<IRuneS<'s>, ITemplataType<'s>> = self.derive_rune_to_type(
      coutputs,
      original_calling_env,
      call_range.to_vec(),
      interface_a.generic_params,
  ...
  pub fn compile_struct_layer(
    ...
    let all_rune_to_type: IndexMap<IRuneS<'s>, ITemplataType<'s>> = self.derive_rune_to_type(
      coutputs,
      outer_env,
      all_ranges.clone(),
      struct_a.generic_params,
    ...
    match self.incrementally_solve(envs, coutputs, &mut solver, |coutputs, solver_state| {
      match get_first_unsolved(struct_a.generic_params, &|rune| {
    ...
    let inner_env_entries: Vec<(INameT<'s, 't>, IEnvEntryT<'s, 't>)> = inferences
      .iter()
      .map(|(rune, templata)| {
  ...
  pub fn compile_interface_layer(
    ...
    let mut solver =
      self.make_solver_state(envs, coutputs, &definition_rules, &rune_to_type, &all_ranges, &[]);
    let get_first_unsolved = |generic_parameters: &'s [&'s GenericParameterS<'s>],
                              is_solved: &dyn Fn(IRuneS<'s>) -> bool| {
      self.get_first_unsolved_identifying_rune(generic_parameters, |rune| is_solved(rune))
    };
    match self.incrementally_solve(envs, coutputs, &mut solver, |coutputs, solver_state| {
      match get_first_unsolved(interface_a.generic_params, &|rune| {
        solver_state.get_conclusion(&rune).is_some()
      }) {
        None => false,
        Some((generic_param, index)) => {
  ...
  pub fn assemble_interface_name(
    ...
    let new_local_name =
      interface_template_name.make_interface_name(self.typing_interner, template_args);
    let steps = template_name.steps();
    *self.typing_interner.intern_id(IdValT {
      package_coord: template_name.package_coord,
      init_steps: &steps,
