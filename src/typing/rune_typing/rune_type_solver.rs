...
pub struct RuneTypeSolveError<'s> {
  ...
  pub failed_solve:
    FailedSolve<IRulexSR<'s>, IRuneS<'s>, ITemplataType<'s>, IRuneTypeRuleError<'s>>,
}
...
impl<'s> IRuneTypeRuleError<'s> {
  pub fn as_lookup_failed(self) -> Option<IRuneTypingLookupFailedError<'s>> {
    match self {
      IRuneTypeRuleError::TooManyMatchingTypes(x) => {
        Some(IRuneTypingLookupFailedError::TooManyMatchingTypes(x))
      }
      IRuneTypeRuleError::CouldntFindType(x) => {
        Some(IRuneTypingLookupFailedError::CouldntFindType(x))
      }
      _ => None,
    }
  }
...
impl<'s> From<IRuneTypingLookupFailedError<'s>> for IRuneTypeRuleError<'s> {
  fn from(e: IRuneTypingLookupFailedError<'s>) -> Self {
    match e {
      IRuneTypingLookupFailedError::TooManyMatchingTypes(x) => {
        IRuneTypeRuleError::TooManyMatchingTypes(x)
      }
...
pub trait IRuneTypeSolverEnv<'s, 't>
where
  's: 't,
{
  fn lookup(
...
impl<'s, 'ctx> RuneTypeSolver<'s, 'ctx> {
  ...
  ) -> Result<IndexMap<IRuneS<'s>, ITemplataType<'s>>, RuneTypeSolveError<'s>>
  where
    's: 't,
  {
    solve_rune_types(
      ...
      rules_s,
      additional_runes,
      expect_complete_solve,
      unpreprocessed_initially_known_runes,
    )
  }
}
...
fn solve_rule<'s, 't, E: IRuneTypeSolverEnv<'s, 't>>(
  ...
  rule: &IRulexSR<'s>,
  coutputs: &CompilerOutputs<'s, 't>,
  solver_state: &mut SimpleSolverState<IRulexSR<'s>, IRuneS<'s>, ITemplataType<'s>>,
) -> Result<(), ISolverError<IRuneS<'s>, ITemplataType<'s>, IRuneTypeRuleError<'s>>> {
  match rule {
    ...
    // }
    // }
    IRulexSR::Resolve(x) => solver_state.commit_step::<IRuneTypeRuleError<'s>>(
      false,
      vec![rule_index],
      [
        (
    ...
    IRulexSR::CallSiteFunc(x) => solver_state.commit_step::<IRuneTypeRuleError<'s>>(
      false,
      vec![rule_index],
      [
        (
          x.prototype_rune.rune.clone(),
          ITemplataType::PrototypeTemplataType(PrototypeTemplataType {}),
        ),
        (
          x.params_list_rune.rune.clone(),
    ...
    IRulexSR::DefinitionFunc(x) => solver_state.commit_step::<IRuneTypeRuleError<'s>>(
      false,
      vec![rule_index],
      [
        (
    ...
    IRulexSR::Equals(x) => {
      ...
      match left_conclusion {
        None => {
          let right_conclusion = solver_state
            .get_conclusion(&x.right.rune)
            .expect("Neither side of EqualsSR has a conclusion");
          solver_state.commit_step::<IRuneTypeRuleError<'s>>(
            false,
            vec![rule_index],
            [(x.left.rune.clone(), right_conclusion)].into_iter().collect(),
            vec![],
            IndexSet::default(),
          )
        }
        Some(left) => solver_state.commit_step::<IRuneTypeRuleError<'s>>(
          false,
          vec![rule_index],
          [(x.right.rune.clone(), left)].into_iter().collect(),
          vec![],
          IndexSet::default(),
        ),
      }
    ...
    // ].into_iter().collect(), vec![], IndexSet::default())
    // }
    IRulexSR::Literal(x) => solver_state.commit_step::<IRuneTypeRuleError<'s>>(
      false,
      vec![rule_index],
      [(x.rune.rune.clone(), x.literal.get_type())].into_iter().collect(),
      vec![],
      IndexSet::default(),
    ),
    IRulexSR::Lookup(x) => {
      ...
      solver_state.commit_step::<IRuneTypeRuleError<'s>>(
        false,
        vec![rule_index],
        [(x.rune.rune.clone(), tyype)].into_iter().collect(),
        vec![],
        IndexSet::default(),
      )
    }
    ...
    IRulexSR::KindList(KindListSR { result_rune, members, .. }) => {
      let mut conclusions: IndexMap<IRuneS<'s>, ITemplataType<'s>> = members
        .iter()
        .map(|m| (m.rune.clone(), ITemplataType::KindTemplataType(KindTemplataType {})))
        .collect();
      conclusions.insert(
    ...
    IRulexSR::OwnRef(OwnRefSR { result_rune, inner_rune, .. }) => solver_state
...
{
  let expected_type =
    solver_state.get_conclusion(&rune.rune).expect("lookup_rune_type: no conclusion for rune");
      ...
      match (&actual_type, &expected_type) {
            ...
            && matches!(tt.return_type, ITemplataType::KindTemplataType(_)) =>
        {
          ...
          match check_generic_call(vec![_range.clone()], &[], &[]) {
            Ok(()) => {}
            Err(e) => {
              return Err(ISolverError::RuleError(RuleError { err: e, _phantom: PhantomData }))
            }
          }
        }
        _ => {
          panic!(
            "lookup_rune_type Templata FoundTemplataDidntMatchExpectedType not yet implemented"
          );
          ...
          match env.lookup(coutputs, lookup.range.clone(), lookup.parts) {
            ...
            Ok(result) => {
              let entries: Vec<(IRuneS<'s>, ITemplataType)> = match &result {
                ...
                IRuneTypeSolverLookupResult::Primitive(p) => match &p.tyype {
                ...
                IRuneTypeSolverLookupResult::Citizen(c) => match &c.tyype {
                  ITemplataType::TemplateTemplataType(t)
                    if t.param_types.is_empty()
                      && matches!(&*t.return_type, ITemplataType::KindTemplataType(_)) =>
                  {
                    vec![]
                  }
                  other => vec![(lookup.rune.rune.clone(), other.clone())],
                },
                IRuneTypeSolverLookupResult::Templata(t) => match &t.templata {
                  ITemplataType::TemplateTemplataType(tt)
                    if tt.param_types.is_empty()
                      && matches!(&*tt.return_type, ITemplataType::KindTemplataType(_)) =>
                  {
                    vec![]
...
fn sanity_check_conclusion<'s>(_rune: IRuneS<'s>, _conclusion: &ITemplataType<'s>) {}
  ...
  for (index, generic_param) in citizen_generic_params.iter().enumerate() {
    if index < arg_types.len() {
      ...
      if generic_param.default.is_some() {
        ...
        return Err(IRuneTypeRuleError::NotEnoughArgumentsForGenericCall(
