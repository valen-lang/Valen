  ...
  's: 't,
{
  ...
  pub fn evaluate_templated_function_from_call_for_prototype(
    ...
    let FunctionTemplataT { outer_env: declaring_env, function_template_id: function_id } =
      function_templata;
    let function = self
      .get_or_create_postparsed_function(coutputs, function_id)
      .expect("an already-resolved function cannot decline");
    if function.is_light() {
      ...
      let lambda_citizen_name_2 = match function.name {
        IFunctionDeclarationNameS::LambdaDeclarationName(lambda_name) => INameT::LambdaCitizen(
          self.typing_interner.alloc(LambdaCitizenNameT {
            template: self
              .typing_interner
              .alloc(LambdaCitizenTemplateNameT { code_location: lambda_name.code_location }),
          }),
        ),
        _ => {
          panic!("vwat");
  ...
  pub fn evaluate_closure_struct(
    ...
    let closured_var_names_and_types: Vec<&'t StructMemberT<'s, 't>> = closured_names
      .iter()
      .map(|name| self.determine_closure_variable_member(containing_node_env, *name))
      .collect();
