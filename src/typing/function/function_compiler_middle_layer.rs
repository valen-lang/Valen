  ...
  's: 't,
{
  ...
  pub fn get_or_evaluate_templated_function_for_banner(
    ...
    let maybe_return_type =
      self.get_maybe_return_type(rued_env, function1.maybe_ret_kind_rune.as_ref().map(|r| &r.rune));
    ...
    let banner =
      FunctionBannerT { origin_function_templata: Some(named_env.templata()), name: named_env.id };

    let signature = self.typing_interner.alloc(SignatureT { id: banner.name });
    match coutputs.lookup_function(signature) {
      Some(function_def) => Ok(PrototypeTemplataT {
        prototype: self.typing_interner.alloc(function_def.header.to_prototype()),
      }),
      None => {
  ...
  pub fn get_or_evaluate_function_for_header(
    ...
    // })
    let rued_env_as_i: IInDenizenEnvironmentT<'s, 't> =
      IInDenizenEnvironmentT::BuildingWithClosuredsAndTemplateArgs(rued_env);
    for generic_param in function1.generic_params {
      ...
      let imprecise_name = self
        .scout_arena
        .intern_imprecise_name(IImpreciseNameValS::RuneName(RuneNameValS { rune: *rune }));
      let mut lookup_filter = HashSet::default();
      lookup_filter.insert(ILookupContext::TemplataLookupContext);
      lookup_filter.insert(ILookupContext::ExpressionLookupContext);
      assert!(rued_env_as_i
    ...
    match coutputs.lookup_function(needle_signature) {
      ...
      None => {
        ...
        let maybe_return_type = self
          .get_maybe_return_type(rued_env, function1.maybe_ret_kind_rune.as_ref().map(|r| &r.rune));
  ...
  pub fn get_maybe_return_type(
    ...
    maybe_ret_coord_rune.map(|ret_coord_rune| {
      ...
      let mut lookup_filter = HashSet::default();
      lookup_filter.insert(ILookupContext::TemplataLookupContext);
      match near_env_as_i.lookup_nearest_with_imprecise_name(
        imprecise_name,
        lookup_filter,
  ...
  pub fn get_generic_function_prototype_from_call(
    ...
    let maybe_return_type = self
      .get_maybe_return_type(rued_env, function1.maybe_ret_kind_rune.as_ref().map(|ru| &ru.rune));
    let named_env =
      self.typing_interner.alloc(self.make_named_env(rued_env, &param_types, maybe_return_type));
    ...
    let params2 =
      self.assemble_function_params(named_env_as_i, coutputs, call_range, function1.params)?;

    let prototype = self.get_function_prototype_for_call(named_env, coutputs, call_range, &params2);
