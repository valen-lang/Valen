  ...
  's: 't,
{
  ...
  pub fn evaluate_function_for_header_core(
    ...
    let _maybe_export = full_env.function.attributes.iter().find_map(|a| match a {
    ...
    let maybe_ret_templata = match &full_env.function.maybe_ret_kind_rune {
      None => None,
      Some(ret_coord_rune) => {
        let imprecise_name =
          self.scout_arena.intern_imprecise_name(IImpreciseNameValS::RuneName(RuneNameValS {
            rune: ret_coord_rune.rune,
          }));
    ...
    let header = match &full_env.function.body {
      IBodyS::CodeBody(_body) => {
        // val attributesWithoutExport = ...
        let attributes_without_export: Vec<&IFunctionAttributeS<'s>> = full_env
          .function
          .attributes
          .iter()
        ...
        match maybe_ret_coord {
          Some(return_coord) => {
            ...
            coutputs.defer_evaluating_function_body(DeferredActionT::EvaluateFunctionBody {
      ...
      IBodyS::ExternBody(_) => {
        ...
        let header = self.make_extern_function(
          coutputs,
          full_env,
      ...
      IBodyS::AbstractBody(_) | IBodyS::GeneratedBody(_) => {
        ...
        let (header, body) = generator.generate_function_body(
          ...
          call_range,
          call_location,
          Some(full_env.function),
          params2,
          maybe_ret_coord,
        )?;

        let header: &'t FunctionHeaderT<'s, 't> = self.typing_interner.alloc(header);
  ...
  pub fn finish_function_maybe_deferred(
    ...
    let header =
      self.finalize_header(full_env_snapshot, coutputs, attributes_t.to_vec(), params_t, ret_coord);
  ...
  pub fn translate_attributes(
    &self,
    attributes_a: &[&IFunctionAttributeS<'s>],
  ...
  pub fn make_extern_function(
    ...
    match env.id.local_name {
      INameT::Function(FunctionNameT {
        ...
        let extern_function_name =
          self.typing_interner.intern_name(INameValT::ExternFunction(ExternFunctionNameValT {
        ...
        let extern_prototype = self
          .typing_interner
          .alloc(PrototypeT { id: *extern_function_id, return_type: header.return_type });
        ...
        coutputs.add_function(header_sig, function2);
        let extern_template_name = self
          .typing_interner
          .intern_extern_template_name(ExternTemplateNameT { code_loc: range.begin });
        ...
        let maybe_inheritance = match ICitizenTemplateNameT::try_from(
          ...
          Ok(_ctn) => {
            let citizen = coutputs
              .lookup_citizen_by_template_name(extern_prototype.id.init_id(self.typing_interner));
            Some(GenericParametersInheritance {
              num_inherited_generic_parameters: citizen.generic_param_types(self.scout_arena).len()
                as i32,
            })
          }
          Err(_) => None,
  ...
  pub fn translate_function_attributes(
    ...
    a.iter()
      .map(|attr| {
        match attr {
          IFunctionAttributeS::UserFunction(_) => IFunctionAttributeT::UserFunction,
          IFunctionAttributeS::Extern(extern_s) => {
            IFunctionAttributeT::Extern(ExternT { package_coord: *extern_s.package_coord })
          }
          _ => {
