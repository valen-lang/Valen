...
use crate::postparsing::names::{DispatcherRuneFromImplValS, IRuneValS};
  ...
  's: 't,
{
  pub fn compile_i_tables(
    // VCOORD: we added this -> Result, but not sure this function should ever return errors, long term
    &self,
  ...
  pub fn make_interface_edge_blueprints(
    ...
    // })
    let x1: Vec<(IdT<'s, 't>, &'t FunctionDefinitionT<'s, 't>)> = coutputs
      .get_all_functions()
      .iter()
      .flat_map(|function| -> Vec<(IdT<'s, 't>, &'t FunctionDefinitionT<'s, 't>)> {
    ...
    let x4: IndexMap<IdT<'s, 't>, Vec<(PrototypeT<'s, 't>, usize)>> = x3
      .into_iter()
      .map(|(interface_template_id, functions)| {
        ...
        let functions_set: HashSet<(SignatureT<'s, 't>, usize)> = functions
          .iter()
          .map(|f| (f.header.to_signature(), f.header.get_virtual_index().expect("vassertSome")))
          .collect();
        ...
        for function in functions.iter() {
          ...
          let already_in_internal = interface_def
            .internal_methods
            .iter()
            .any(|(p, _)| p.to_signature() == prototype.to_signature());
          if !already_in_internal {
            let virtual_index =
              header.get_virtual_index().expect("vassertSome: getVirtualIndex for abstract header");
    ...
    abstract_function_headers
      .into_iter()
      .map(|(interface_template_id, function_headers)| -> &'t InterfaceEdgeBlueprintT<'s, 't> {
  ...
  pub fn create_override_placeholder_mimicking(
    ...
    let placeholder_name =
      self.typing_interner.intern_kind_placeholder_name(KindPlaceholderNameT {
        template: self
          .typing_interner
    ...
    match original_templata_to_mimic {
      ...
      other => {
        panic!("vwat: create_override_placeholder_mimicking unexpected templata: {:?}", other)
      }
    }
  }
  ...
  pub fn look_for_override(
    ...
    let abstract_func_template_id =
      Compiler::get_function_template(self.typing_interner, abstract_function_prototype.id);
    let abstract_function_param_unsubstituted_types = abstract_function_prototype.param_types();
    assert!(abstract_index >= 0);
    let abstract_param_unsubstituted_type =
      abstract_function_param_unsubstituted_types[abstract_index as usize];

    let maybe_origin_function_templata = coutputs
      .lookup_function(self.typing_interner.alloc(abstract_function_prototype.to_signature()))
    ...
    let dispatcher_template_id_ref = abstract_func_template_id
      .add_step(self.typing_interner, INameT::OverrideDispatcherTemplate(dispatcher_template_name));
    ...
    let impl_placeholder_to_dispatcher_placeholder: Vec<(IdT<'s, 't>, ITemplataT<'s, 't>)> =
      instantiated_local
        ...
        .map(|(impl_placeholder_index, impl_placeholder)| {
          ...
          let impl_placeholder_local_name =
            IPlaceholderNameT::try_from(impl_placeholder_id.local_name).unwrap_or_else(|_| {
              panic!("vwat: expected IPlaceholderNameT for impl placeholder local_name")
            });
          ...
          match impl_placeholder_id.init_steps.last() {
            Some(name) => {
              IImplTemplateNameT::try_from(*name).unwrap_or_else(|_| {
                panic!("vwat: last init step should be IImplTemplateNameT, got {:?}", name)
              });
            }
            None => panic!("vwat: last init step should be IImplTemplateNameT, got None"),
          }
          let dispatcher_rune = self.scout_arena.intern_rune(IRuneValS::DispatcherRuneFromImpl(
            DispatcherRuneFromImplValS { inner_rune: impl_rune },
          ));
    ...
    let dispatcher_params: Vec<KindT<'s, 't>> = origin_func
      .params
      .iter()
      .map(|p| p.full_type_rune.rune)
      .map(|rune| {
        let templata = *dispatcher_inner_inferences.get(&rune).unwrap_or_else(|| {
          panic!("vassertSome: rune {:?} not in dispatcherInnerInferences", rune)
        });
      ...
      .filter_map(|gp| {
        dispatcher_inner_inferences.get(&gp.rune.rune).and_then(|templata| match *templata {
          ITemplataT::Kind(&KindTemplataT {
            ...
            if existing_dispatcher_placeholder_ids.contains(&id) {
              None
            } else {
              Some(*templata)
            }
    ...
    let dispatcher_inner_env: &'t GeneralEnvironmentT<'s, 't> = child_of(
      ...
      *dispatcher_template_id_ref,
      ...
      dispatcher_inner_inferences
        .iter()
        .map(|(name_s, templata): (&IRuneS<'s>, &ITemplataT<'s, 't>)| {
    ...
    let impl_independent_rune_to_impl_placeholder_and_case_placeholder: Vec<(
      IRuneS<'s>,
      IdT<'s, 't>,
      ITemplataT<'s, 't>,
    )> = impl_a
      ...
      .map(|(index, (impl_rune, impl_placeholder_templata))| {
        let case_rune = self
          .scout_arena
          .intern_rune(IRuneValS::CaseRuneFromImpl(CaseRuneFromImplValS { inner_rune: impl_rune }));
    ...
    let impl_independent_rune_to_case_placeholder: Vec<(IRuneS<'s>, ITemplataT<'s, 't>)> =
      impl_independent_rune_to_impl_placeholder_and_case_placeholder
        .iter()
    ...
    let partial_resolve_conclusions = self
      .partial_resolve_impl(
        coutputs,
        &[range],
        call_location,
        IInDenizenEnvironmentT::from(dispatcher_inner_env),
        &{
          ...
          for (rune, templata) in impl_independent_rune_to_case_placeholder.iter() {
            knowns
              .push(InitialKnown { rune: RuneUsage { range, rune: *rune }, templata: *templata });
          }
          knowns
        },
        impl_t.templata,
    ...
    let dispatcher_inner_env_with_bounds_for_sub_citizen: &'t GeneralEnvironmentT<'s, 't> =
      child_of(
        self.typing_interner,
        self.scout_arena,
        IInDenizenEnvironmentT::from(dispatcher_inner_env),
        *dispatcher_template_id_ref,
        ...
        dispatcher_and_case_placeholdered_impl_reachable_prototypes
          ...
          .map(
            |(
              index,
              (_rune_in_impl, _rune_in_citizen, dispatcher_placeholdered_reachable_prototype),
            )| {
              let reachable_prototype_name = self
                .typing_interner
                .intern_reachable_prototype_name(ReachablePrototypeNameT { num: index as i32 });
              (
                INameT::ReachablePrototype(reachable_prototype_name),
                IEnvEntryT::Templata(ITemplataT::Prototype(self.typing_interner.alloc(
                  PrototypeTemplataT {
                    prototype: dispatcher_placeholdered_reachable_prototype.prototype,
                  },
                ))),
              )
            },
    ...
    let resolve_result = self.resolve_impl(
      ...
      &{
        ...
        for (rune, templata) in impl_independent_rune_to_case_placeholder.iter() {
          knowns.push(InitialKnown { rune: RuneUsage { range, rune: *rune }, templata: *templata });
        }
        knowns
      },
      impl_t.templata,
    ...
    let (impl_conclusions, _impl_instantiation_bound_args_unused) = match resolve_result {
      Ok(CompleteResolveSolve { conclusions, rune_to_bound }) => (conclusions, rune_to_bound),
    ...
    let dispatcher_case_placeholdered_sub_citizen: ICitizenTT<'s, 't> = {
      let templata = impl_conclusions
        .get(&impl_a.struct_kind_rune.rune)
    ...
    let override_imprecise_name =
      get_imprecise_name(self.scout_arena, abstract_function_prototype.id.local_name)
    ...
    let dispatcher_case_name =
      self.typing_interner.intern_override_dispatcher_case_name(OverrideDispatcherCaseNameValT {
        independent_impl_template_args: &case_placeholder_templatas,
      });
    let dispatcher_case_id_ref = dispatcher_id_ref
      .add_step(self.typing_interner, INameT::OverrideDispatcherCase(dispatcher_case_name));
    ...
    assert!(coutputs
      .get_instantiation_bounds(self.typing_interner, found_function.prototype.id)
      .is_some());
    ...
    let reachable_map = {
      let mut grouped: HashMap<IRuneS<'s>, Vec<(IRuneS<'s>, PrototypeT<'s, 't>)>> =
        HashMap::default();
      for (rune_in_impl, rune_in_citizen, prototype_templata) in
        &dispatcher_and_case_placeholdered_impl_reachable_prototypes
      {
        grouped
          .entry(*rune_in_impl)
          .or_default()
          .push((*rune_in_citizen, *prototype_templata.prototype));
      }
      self.typing_interner.alloc_index_map_from_iter(grouped.into_iter().map(
        |(rune_in_impl, inner_entries)| {
          let inner_map: ArenaIndexMap<'t, IRuneS<'s>, PrototypeT<'s, 't>> =
            self.typing_interner.alloc_index_map_from_iter(inner_entries.into_iter());
