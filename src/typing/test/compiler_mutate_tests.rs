...
use crate::typing::types::types::{
  IntT, KindT, RegionT, ShareRefT, SharednessT, StaticSizedArrayTT, StructTTValT,
};
...
"#;
  ...
  match &err {
    ICompileErrorT::CouldntConvertForMutateT {
      expected_type: KindT::Int(IntT { bits: 32 }),
      actual_type: KindT::Bool(_),
      ..
    } => {}
...
";
  ...
  match &err {
    ICompileErrorT::CantImplNonInterface {
      templata: ITemplataT::Kind(KindTemplataT { kind: KindT::Int(IntT { bits: 32 }) }),
      ..
    } => {}
...
";
  ...
  let err = compile.get_compiler_outputs().err().unwrap_or_else(|| {
    panic!("expected Err(RangedInternalErrorT) for if-branch-move-mismatch, got Ok")
  });
  match &err {
...
";
  ...
  match compile.get_compiler_outputs() {
    Ok(_) => {}
    Err(e) => panic!(
      "expected Ok (same vars moved in both branches, just different order), got Err: {:?}",
      e
    ),
  }
}
...
fn humanize_errors() {
  ...
  let myfunc_func_name = typing_interner.intern_function_name(FunctionNameValT {
  ...
  let myfunc_id = typing_interner.intern_id(IdValT {
    package_coord: test_tld,
    init_steps: &[],
    local_name: INameT::Function(myfunc_func_name),
  });
  ...
  assert!(!humanize(
    ...
    ICompileErrorT::CantReconcileBranchesResults {
      range: tz_slice,
      then_result: firefly_coord,
      else_result: serenity_coord
    }
  )
  .is_empty());
