  ...
  's: 't,
{
  ...
  pub function_declared_names: HashMap<IdT<'s, 't>, RangeS<'s>>,
  pub type_declared_names: HashSet<IdT<'s, 't>>,

  pub function_name_to_outer_env: HashMap<IdT<'s, 't>, IInDenizenEnvironmentT<'s, 't>>,
  pub function_name_to_inner_env: HashMap<IdT<'s, 't>, IInDenizenEnvironmentT<'s, 't>>,
  ...
  pub all_impls: HashMap<IdT<'s, 't>, &'t ImplT<'s, 't>>,
  pub sub_citizen_template_to_impls: HashMap<IdT<'s, 't>, Vec<&'t ImplT<'s, 't>>>,
  pub super_interface_template_to_impls: HashMap<IdT<'s, 't>, Vec<&'t ImplT<'s, 't>>>,
  ...
  pub finished_deferred_function_compiles: HashSet<IdT<'s, 't>>,
}
  ...
  's: 't,
{
  ...
  pub fn add_instantiation_bounds(
    ...
    for (_rune, reachable_bound_args) in
      &instantiation_bound_args.rune_to_citizen_rune_to_reachable_prototype
    {
      for (_callee_rune, reachable_prototype) in
        &reachable_bound_args.citizen_rune_to_reachable_prototype
      {
    ...
    if let Some(existing) = self.instantiation_name_to_bounds.get(instantiation_id_ref) {
      ...
      assert!(
        existing.rune_to_bound_prototype == instantiation_bound_args.rune_to_bound_prototype
          && existing.rune_to_citizen_rune_to_reachable_prototype
            == instantiation_bound_args.rune_to_citizen_rune_to_reachable_prototype
  ...
  pub fn add_function_export(
    ...
    let export =
      interner.alloc(FunctionExportT { range, prototype: *function, export_id, exported_name });
  ...
  pub fn add_function_extern(
    ...
    let function_extern = interner.alloc(FunctionExternT {
      range,
      extern_placeholdered_id,
      prototype: *function,
      extern_name: exported_name,
      generic_parameter_inheritance,
    });
    self.function_externs.push(function_extern);
  }

  pub fn defer_evaluating_function_body(&mut self, devf: DeferredActionT<'s, 't>) {
  ...
  pub fn lookup_struct_template(
    ...
    *self.struct_template_name_to_definition.get(&template_name).expect("Struct template not found")
  }
  ...
  pub fn lookup_interface_by_template_name(
    ...
    match self.interface_template_name_to_definition.get(&template_name) {
      None => {
        panic!("vfail: vassertSome: lookupInterface templateName not found: {:?}", template_name)
      }
      Some(d) => *d,
    }
  }
  ...
  pub fn lookup_citizen_by_template_name(
    ...
    match template_name.local_name {
      INameT::AnonymousSubstructTemplate(_) => {
        CitizenDefinitionT::Struct(self.lookup_struct_template(template_name))
      }
      INameT::StructTemplate(_) => {
        CitizenDefinitionT::Struct(self.lookup_struct_template(template_name))
      }
      INameT::InterfaceTemplate(_) => {
        CitizenDefinitionT::Interface(self.lookup_interface_by_template_name(template_name))
      }
      _ => panic!(
        "lookup_citizen_by_template_name: unexpected local_name variant: {:?}",
        template_name
      ),
    }
  }
  ...
  pub(in crate::typing) fn register_postparsed_function(
    &mut self,
    template_id: &'t IdT<'s, 't>,
  ...
  pub(in crate::typing) fn get_postparsed_interface(
    &self,
    template_id: &'t IdT<'s, 't>,
  ) -> &'s InterfaceS<'s> {
    self
  ...
  pub(in crate::typing) fn get_postparsed_impl(
    &self,
    template_id: &'t IdT<'s, 't>,
  ) -> &'s ImplS<'s> {
    self
