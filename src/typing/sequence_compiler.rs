  ...
  's: 't,
{
  ...
  pub fn make_tuple_kind(
    ...
    let tuple_template_name =
      self.typing_interner.intern_struct_template_name(StructTemplateNameT {
        human_name: self.keywords.tuple_human_name[types.len()],
      });
    let tuple_template = match env
      .lookup_nearest_with_name(
        INameT::StructTemplate(tuple_template_name),
        {
    ...
    let new_parent_ranges: Vec<RangeS<'s>> = once(RangeS::internal(self.scout_arena, -17653))
      .chain(parent_ranges.iter().copied())
      .collect();
    ...
    match self.resolve_struct(
      coutputs,
      env,
      self.typing_interner.alloc_slice_from_vec(new_parent_ranges),
      call_location,
      tuple_template,
