  ...
  Conclusion: Clone + PartialEq,
{
  ...
  pub fn commit_step<ErrType>(
    ...
    let solved_rules: Vec<(i32, Rule)> =
      solved_rule_indices.iter().map(|&idx| (idx, self.rules[idx as usize].clone())).collect();
    let step = super::Step {
      complex,
      solved_rules,
    ...
    for (rune, new_conclusion) in &conclusions {
      if let Some(existing) = self.rune_to_conclusion.get(rune) {
        if existing != new_conclusion {
          return Err(super::ISolverError::SolverConflict(super::SolverConflict {
            rune: rune.clone(),
    ...
    for rule in new_rules {
      ...
      self.sanity_check();
      for puzzle in puzzles {
        let entry = self.open_rule_to_puzzle_to_runes.entry(rule_index).or_insert_with(Vec::new);
        entry.push(puzzle);
      }
      self.sanity_check();
    }
  ...
  pub fn get_next_solvable(&self) -> Option<i32> {
    self
      ...
      .filter(|(_, puzzle_to_runes)| {
        puzzle_to_runes
          .iter()
          .any(|runes| runes.iter().all(|r| self.rune_to_conclusion.contains_key(r)))
      })
  ...
  pub fn get_unsolved_rules(&self) -> Vec<Rule> {
    self.open_rule_to_puzzle_to_runes.keys().map(|&idx| self.rules[idx as usize].clone()).collect()
  }

  pub fn get_unsolved_runes(&self) -> Vec<Rune> {
    self.all_runes.iter().filter(|r| !self.rune_to_conclusion.contains_key(*r)).cloned().collect()
  }
  ...
  pub fn new(rule_to_puzzles: Box<dyn Fn(&Rule) -> Vec<Vec<Rune>>>, all_runes: Vec<Rune>) -> Self {
    SimpleSolverState {
      rule_to_puzzles,
      steps: vec![],
      rules: vec![],
