...
impl<'ctx, 's> TestRuleSolver<'ctx, 's> {
  pub fn complex_solve_impl(
    ...
    for receiver in receiver_runes {
      ...
      let call_templates: Vec<String> = call_rules
        ...
        .collect();
      let any_unknown_constraints =
        sender_conclusions.len() != receive_rules.len() || call_rules.len() != call_templates.len();
      if let Some(receiver_instantiation) =
        self.solve_receives(sender_conclusions, call_templates, any_unknown_constraints)
      {
    ...
    match rule {
      TestRule::Equals(Equals { left_rune, right_rune }) => {
        match solver_state.get_conclusion(left_rune) {
          Some(left) => solver_state.commit_step::<String>(
            false,
            vec![rule_index],
            [(*right_rune, left)].into_iter().collect(),
            vec![],
            IndexSet::default(),
          ),
          None => {
            let right =
              solver_state.get_conclusion(right_rune).expect("right rune must have conclusion");
            solver_state.commit_step::<String>(
              false,
              vec![rule_index],
              [(*left_rune, right)].into_iter().collect(),
              vec![],
              IndexSet::default(),
            )
          }
      ...
      TestRule::Lookup(Lookup { rune, name }) => solver_state.commit_step::<String>(
        false,
        vec![rule_index],
        [(*rune, name.clone())].into_iter().collect(),
        vec![],
        IndexSet::default(),
      ),
      TestRule::Literal(Literal { rune, value }) => solver_state.commit_step::<String>(
        false,
        vec![rule_index],
        [(*rune, value.clone())].into_iter().collect(),
        vec![],
        IndexSet::default(),
      ),
      TestRule::OneOf(OneOf { coord_rune, possible_values }) => {
        let literal =
          solver_state.get_conclusion(coord_rune).expect("OneOf rune must have conclusion");
      ...
      TestRule::CoordComponents(CoordComponents { coord_rune, ownership_rune, kind_rune }) => {
        match solver_state.get_conclusion(coord_rune) {
          Some(combined) => {
            ...
            solver_state.commit_step::<String>(
              false,
              vec![rule_index],
              [(*ownership_rune, ownership), (*kind_rune, kind)].into_iter().collect(),
              vec![],
              IndexSet::default(),
            )
          }
          None => {
            let ownership =
              solver_state.get_conclusion(ownership_rune).expect("ownership required");
            let kind = solver_state.get_conclusion(kind_rune).expect("kind required");
            let combined = format!("{}/{}", ownership, kind);
            solver_state.commit_step::<String>(
              false,
              vec![rule_index],
              [(*coord_rune, combined)].into_iter().collect(),
              vec![],
              IndexSet::default(),
            )
          }
      ...
      TestRule::Pack(Pack { result_rune, member_runes }) => {
        match solver_state.get_conclusion(result_rune) {
          Some(result) => {
            let parts: Vec<&str> = result.split(',').collect();
            let conclusions: IndexMap<i64, String> = member_runes
              .iter()
              .zip(parts.iter())
          ...
          None => {
            let result: String = member_runes
              .iter()
              .map(|r| solver_state.get_conclusion(r).expect("member rune must have conclusion"))
              .collect::<Vec<_>>()
              .join(",");
            solver_state.commit_step::<String>(
              false,
              vec![rule_index],
              [(*result_rune, result)].into_iter().collect(),
              vec![],
              IndexSet::default(),
            )
          }
      ...
      TestRule::Call(Call { result_rune, name_rune, arg_rune }) => {
        ...
        match (maybe_result, maybe_name, maybe_arg) {
          (Some(result), Some(template_name), _) => {
            ...
            solver_state.commit_step::<String>(
              false,
              vec![rule_index],
              [(*arg_rune, arg)].into_iter().collect(),
              vec![],
              IndexSet::default(),
            )
          }
      ...
      TestRule::Send(Send { sender_rune, receiver_rune }) => {
        let receiver =
          solver_state.get_conclusion(receiver_rune).expect("receiver must have conclusion");
        if receiver == "ISpaceship" || receiver == "IWeapon:int" {
          let new_rule =
            TestRule::Implements(Implements { sub_rune: *sender_rune, super_rune: *receiver_rune });
          solver_state.commit_step::<String>(
            false,
            vec![rule_index],
            IndexMap::default(),
          ...
          solver_state.commit_step::<String>(
            false,
            vec![rule_index],
            [(*sender_rune, receiver)].into_iter().collect(),
            vec![],
            IndexSet::default(),
          )
        }
      ...
      TestRule::Implements(Implements { sub_rune, super_rune }) => {
        let sub = solver_state.get_conclusion(sub_rune).expect("sub must have conclusion");
        let suuper = solver_state.get_conclusion(super_rune).expect("super must have conclusion");
  ...
  fn solve_receives(
    ...
    let sender_ancestor_lists: Vec<Vec<String>> =
      sender_templates.iter().map(|s| self.get_ancestors(s, true)).collect();
    let common_ancestors: HashSet<String> = sender_ancestor_lists
      ...
      .unwrap_or_default();
    let call_templates_set: HashSet<String> = call_templates.iter().cloned().collect();
    let common_ancestors_call_constrained = if call_templates_set.is_empty() {
      common_ancestors
    } else {
      common_ancestors.intersection(&call_templates_set).cloned().collect()
    };
    ...
    if common_ancestors_narrowed.is_empty() {
      ...
      let ancestor_template = common_ancestors_narrowed.iter().next().expect("non-empty");
