...
impl IRule for CoordComponents {
  ...
  fn all_puzzles(&self) -> Vec<Vec<i64>> {
    vec![vec![self.coord_rune], vec![self.ownership_rune, self.kind_rune]]
  }
}
...
impl IRule for Call {
  ...
  fn all_puzzles(&self) -> Vec<Vec<i64>> {
    vec![vec![self.result_rune, self.name_rune], vec![self.name_rune, self.arg_rune]]
  }
}
...
impl IRule for Pack {
  fn all_runes(&self) -> Vec<i64> {
    vec![self.result_rune].into_iter().chain(self.member_runes.iter().cloned()).collect()
  }
  fn all_puzzles(&self) -> Vec<Vec<i64>> {
    if self.member_runes.is_empty() {
      vec![vec![self.result_rune]]
    } else {
      vec![vec![self.result_rune], self.member_runes.clone()]
    }
  }
}
