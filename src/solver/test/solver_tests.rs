...
use crate::solver::{make_solver_state, FailedSolve, ISolverError, SimpleSolverState};
...
fn simple_int_rule() {
  let rules: Vec<TestRule> =
    vec![TestRule::Literal(Literal { rune: -1, value: "1337".to_string() })];
  let result = get_conclusions(rules, true, IndexMap::default());
...
fn equals_transitive() {
  let rules: Vec<TestRule> = vec![
    TestRule::Equals(Equals { left_rune: -2, right_rune: -1 }),
...
fn incomplete_solve() {
  ...
  let result = get_conclusions(rules, false, IndexMap::default());
...
fn half_complete_solve() {
  let rules: Vec<TestRule> = vec![
    ...
    TestRule::Literal(Literal { rune: -2, value: "1337".to_string() }),
  ];
  let result = get_conclusions(rules, false, IndexMap::default());
  let expected: HashMap<i64, String> = [(-2, "1337".to_string())].into_iter().collect();
  assert_eq!(result, expected);
}
...
fn solves_a_components_rule() {
  let rules: Vec<TestRule> = vec![
    ...
    TestRule::Literal(Literal { rune: -2, value: "1337".to_string() }),
    TestRule::Literal(Literal { rune: -3, value: "1448".to_string() }),
  ];
  let result = get_conclusions(rules, true, IndexMap::default());
  let expected: HashMap<i64, String> =
    [(-1, "1337/1448".to_string()), (-2, "1337".to_string()), (-3, "1448".to_string())]
      .into_iter()
      .collect();
  assert_eq!(result, expected);
}
...
fn reverse_solve_a_components_rule() {
  let rules: Vec<TestRule> = vec![
    ...
    TestRule::Literal(Literal { rune: -1, value: "1337/1448".to_string() }),
  ];
  let result = get_conclusions(rules, true, IndexMap::default());
  let expected: HashMap<i64, String> =
    [(-1, "1337/1448".to_string()), (-2, "1337".to_string()), (-3, "1448".to_string())]
...
fn test_infer_pack() {
  let rules: Vec<TestRule> = vec![
    ...
    TestRule::Pack(Pack { result_rune: -3, member_runes: vec![-1, -2] }),
  ];
  let result = get_conclusions(rules, true, IndexMap::default());
  let expected: HashMap<i64, String> =
    [(-1, "1337".to_string()), (-2, "1448".to_string()), (-3, "1337,1448".to_string())]
      .into_iter()
      .collect();
  assert_eq!(result, expected);
}
...
fn test_infer_pack_from_result() {
  let rules: Vec<TestRule> = vec![
    TestRule::Literal(Literal { rune: -3, value: "1337,1448".to_string() }),
    TestRule::Pack(Pack { result_rune: -3, member_runes: vec![-1, -2] }),
  ];
  let result = get_conclusions(rules, true, IndexMap::default());
  let expected: HashMap<i64, String> =
...
fn test_infer_pack_from_empty_result() {
  let rules: Vec<TestRule> = vec![
    TestRule::Literal(Literal { rune: -3, value: "".to_string() }),
    TestRule::Pack(Pack { result_rune: -3, member_runes: vec![] }),
  ];
  let result = get_conclusions(rules, true, IndexMap::default());
...
fn test_cant_solve_empty_pack() {
  let rules: Vec<TestRule> = vec![TestRule::Pack(Pack { result_rune: -3, member_runes: vec![] })];
  let result = get_conclusions(rules, false, IndexMap::default());
  let expected: HashMap<i64, String> = HashMap::default();
  assert_eq!(result, expected);
}
...
fn complex_rule_set() {
  let rules: Vec<TestRule> = vec![
    ...
    TestRule::Literal(Literal { rune: -2, value: "1337".to_string() }),
    TestRule::Equals(Equals { left_rune: -4, right_rune: -2 }),
    TestRule::OneOf(OneOf {
      coord_rune: -4,
      possible_values: vec!["1337".to_string(), "73".to_string()],
    }),
    TestRule::Equals(Equals { left_rune: -1, right_rune: -5 }),
    TestRule::CoordComponents(CoordComponents {
      coord_rune: -1,
      ownership_rune: -2,
      kind_rune: -3,
    }),
    TestRule::Equals(Equals { left_rune: -6, right_rune: -7 }),
  ];
  let conclusions = get_conclusions(rules, true, IndexMap::default());
  assert_eq!(conclusions.get(&-7), Some(&"1337/1448/1337/1448".to_string()));
}

#[test]
fn test_receiving_struct_to_struct() {
  let rules: Vec<TestRule> = vec![
    TestRule::Literal(Literal { rune: -1, value: "Firefly".to_string() }),
    TestRule::Send(Send { sender_rune: -2, receiver_rune: -1 }),
  ];
  let result = get_conclusions(rules, true, IndexMap::default());
  let expected: HashMap<i64, String> =
    [(-1, "Firefly".to_string()), (-2, "Firefly".to_string())].into_iter().collect();
  assert_eq!(result, expected);
}
...
fn test_receive_struct_from_sent_interface() {
  let rules: Vec<TestRule> = vec![
    TestRule::Literal(Literal { rune: -1, value: "Firefly".to_string() }),
    TestRule::Literal(Literal { rune: -2, value: "ISpaceship".to_string() }),
    TestRule::Send(Send { sender_rune: -2, receiver_rune: -1 }),
  ];
  let failed = expect_solve_failure(rules);

  let conclusions_set: HashSet<(i64, String)> =
    failed.steps.iter().flat_map(|s| s.conclusions.iter().map(|(r, c)| (*r, c.clone()))).collect();
  let expected_conclusions: HashSet<(i64, String)> =
    [(-1, "Firefly".to_string()), (-2, "ISpaceship".to_string()), (-2, "Firefly".to_string())]
      .into_iter()
      .collect();
...
fn test_receive_interface_from_sent_struct() {
  let rules: Vec<TestRule> = vec![
    TestRule::Literal(Literal { rune: -1, value: "ISpaceship".to_string() }),
    TestRule::Literal(Literal { rune: -2, value: "Firefly".to_string() }),
    TestRule::Send(Send { sender_rune: -2, receiver_rune: -1 }),
  ];
  let result = get_conclusions(rules, true, IndexMap::default());
  let expected: HashMap<i64, String> =
    [(-1, "ISpaceship".to_string()), (-2, "Firefly".to_string())].into_iter().collect();
  assert_eq!(result, expected);
}
...
fn test_complex_solve_calculate_common_ancestor() {
  let rules: Vec<TestRule> = vec![
    TestRule::Literal(Literal { rune: -2, value: "Firefly".to_string() }),
    TestRule::Literal(Literal { rune: -3, value: "Serenity".to_string() }),
    TestRule::Send(Send { sender_rune: -2, receiver_rune: -1 }),
    TestRule::Send(Send { sender_rune: -3, receiver_rune: -1 }),
  ];
  let result = get_conclusions(rules, true, IndexMap::default());
  let expected: HashMap<i64, String> =
    [(-1, "ISpaceship".to_string()), (-2, "Firefly".to_string()), (-3, "Serenity".to_string())]
      .into_iter()
      .collect();
  assert_eq!(result, expected);
}
...
fn test_complex_solve_descendant_satisfying_call() {
  let rules: Vec<TestRule> = vec![
    TestRule::Literal(Literal { rune: -2, value: "Flamethrower:int".to_string() }),
    TestRule::Send(Send { sender_rune: -2, receiver_rune: -1 }),
    TestRule::Call(Call { result_rune: -1, name_rune: -3, arg_rune: -4 }),
    TestRule::Literal(Literal { rune: -3, value: "IWeapon".to_string() }),
  ];
  let result = get_conclusions(rules, true, IndexMap::default());
  let expected: HashMap<i64, String> = [
...
fn partial_solve() {
  ...
  let rules: Vec<TestRule> = vec![
    ...
    TestRule::Call(Call { result_rune: -3, name_rune: -1, arg_rune: -2 }),
  ];
  ...
  let mut new_conclusions = IndexMap::default();
  new_conclusions.insert(-1i64, "Firefly".to_string());
  solver_state
    .commit_step::<String>(false, vec![], new_conclusions, vec![], IndexSet::default())
    .expect("commit_step");
...
fn predicting() {
  ...
  let expected: HashMap<i64, String> =
    [(-1, "Firefly".to_string()), (-2, "1337".to_string()), (-3, "Firefly:1337".to_string())]
      .into_iter()
      .collect();
...
fn solve_with_puzzler(
  ...
  let scout_arena = ScoutArena::new(&scout_bump);
  let rules: Vec<TestRule> = vec![
    TestRule::Lookup(Lookup { rune: -1, name: "Firefly".to_string() }),
    TestRule::Literal(Literal { rune: -2, value: "1337".to_string() }),
    TestRule::Call(Call { result_rune: -3, name_rune: -1, arg_rune: -2 }),
  ];
  let all_runes: Vec<i64> = {
...
fn test_conflict() {
  let rules: Vec<TestRule> = vec![
    TestRule::Literal(Literal { rune: -1, value: "1448".to_string() }),
    TestRule::Literal(Literal { rune: -1, value: "1337".to_string() }),
  ];
  let failed = expect_solve_failure(rules);
  match &failed.error {
...
fn get_conclusions(
  ...
  let scout_bump = Bump::new();
  let scout_arena = ScoutArena::new(&scout_bump);
  let all_runes_from_rules: HashSet<i64> = rules.iter().flat_map(|r| r.all_runes()).collect();
  let all_runes: Vec<i64> = {
  ...
  let conclusions: HashMap<i64, String> = solver_state.userify_conclusions().into_iter().collect();
  let conclusions_keys: HashSet<i64> = conclusions.keys().cloned().collect();
