...
The **region borrow checker** — ladder, quarantine rule, and rustc evidence are all recorded below under "Region borrow checker". Rung 0 (groups become real) is entirely architect work and is gated on decision 1 above.
...
- **`ITemplataT`'s region payload is the group algebra**, and it names a local's group as readily as a parameter's. A group *parameter* is a `PlaceholderTemplataT` like any other generic parameter — that path already exists. A *concrete* group expression needs the new variant.
- **►► A REGION IS NEVER `mut` OR `imm` ◄◄** A region says *which places*, never *what you may do to them*. Load-bearing: put permission on the region and `read_hp(e &Entity in g)` and `heal(e &Entity in g) mut(g)` stop having the same parameter type, and the next question is variance — which Valen refused deliberately and everywhere. **Condemned as fossils by this pin:** `IRegionMutabilityS::{ReadOnlyRegion, ReadWriteRegion}`; `imm` as a *region modifier*; and `RegionT::Iso` (isolation is a property of how a group was *minted*, not a variant of the algebra).
- **Borrow creation computes, it does not check** *(computation half)* — the group lives in the type, on the `BorrowRef`'s region slot, which is exactly where all 9 construction sites already stamp `Default`. Confirmed compatible with answer 20; the checking half is the joint-argument check at call sites.
- **A borrow-of-claim must carry the claim's member-level `rc.T` mention** *(ruled upstream; fail-closed non-Send, never mutation-poisoned)*. One carrying no multi mention escapes Send, type-scoping, isolation and erasure coverage — a captured claim borrow crossing a persistent executor is a use-after-free plus a non-atomic RC. Our `BorrowRef(ShareRef(…))` carries only its region slot, so this is a rung-0 representation input, not current work.
- **Effect derivation: the solver produces the bindings; the effect falls out by SUBSTITUTION afterward.** Not held in the solver, not a rune conclusion. Zero solver changes in principle — but see the two gaps above (args don't reach the solve; `substitute_templatas_in_kind` unimplemented for ref wraps). Neither is *extra* cost; both are on the board regardless.
- **Effect representation is UNSETTLED.** Sequence of attempts, recorded so nobody re-walks it: (i) `ITemplataT::Effect` with a folded `mutates` plus an `includes` list — dropped; (ii) **"effects as a bound family" — REFUTED**, bounds are denizen lookups (see above); (iii) re-derived as a header field plus a `NotMutSR` rule variant discharged in **`check_resolving_conclusions_and_resolve`** — **the discharge site is confirmed good by the architect**, and it is the right home because regions collect-and-fold so no value exists mid-solve; but (iv) answer 21 says a bare `mutates: RegionT` is **too narrow** — the permission axis is real. **The live candidate is the per-group permission map** (originally "Alt 3"), keyed by group expression, valued by a set of promises rather than an ordered level. Cost to remember: map keys are group *expressions*, so it needs an eager canonical form for the algebra.
- **`not(mut(…))` applies to the WHOLE CALL, not to a named subject.** So the rule variant needs no `subject_rune` — just the banned group expression. Consequences: the callback constraint stops being a special case (a forwarded `mut(E)` is part of the call's total effect, so it's reached automatically); the ancestors-banned/descendants-free asymmetry is just the reach rule; and Pin C reads off it directly, since the "fully-solved value" is the call's total effect and only exists post-fold. **Per answer 23, the negatives are a list of independent conjuncts, not one folded expression.**
- **A monotone dataflow analysis iterates over the FINISHED TREE, never the compilation.** Re-walking a `While` sub-tree is a walk; recompiling would re-resolve overloads and re-allocate into the arena. Lowering to a CFG stays rejected (a parallel fact-IR was dropped for sync cost, and rustc had to *edit MIR for borrowck after the fact* to blur back the CFG structure it bought). **Confirmed feasible** — control flow survives, see above.
...
The endeavor: a borrow checker as **the last phase at each call site** — phase 5 of six; see the CALL-SITE PHASES section for the full pipeline. Target program is a `Vec<Ship>` with two borrows into one group both passed to one `attack`, surviving mutation, then dying when the container is cleared. Sources: `/Volumes/V/LangNotesValen/Valen/valen-design-1.md` (the authority), plus two surveys of `~/rust`.

**Regions are INERT CARGO through type-solving.** The load-bearing claim is "nothing mid-solve needs to know *which* group a region is," and the corpus states it rather than implying it: design-1:1124 — ***"Groups themselves don't conform to traits"*** (so no trait resolution ever depends on a group's value) — and design-1:1118 — *"`T` alone in a signature is group-agnostic. Only `&Foo in g` carries group information. Group parameters constrain the pointee type via a **separate parameter**"* (`func serialize_all<T: Serializable, g': T>(items: &Vec<T> in g)` — the pointee carries the bound, the group carries nothing).

So the solver should **never put regions into conflict detection.** It accumulates the set of group expressions bound to each region rune, and a later fold applies the widening rule (merge iff no binding with a narrower claim survives). Union is commutative and associative, so the fold is order-independent — no merge-on-conflict special case, and the exact-binding rejection becomes a *cardinality check* rather than a bolted-on arm. This matches rustc, where `relate_tys.rs:419` — the analogue of our unification — never unifies regions and **is infallible**, and HIR typeck erases regions entirely on writeback. Every place a group's value *is* read is post-solve or in another domain: binding-site disjointness (at the call site), erasure coverage (post-solve), temp unions (merge-on-accumulate, native to this design), effect solving, and poisoning (flow analysis).
- **Store provenance, not bare group expressions.** Collect `(group expression, contributing site)` pairs so the fold can point at the right argument. rustc pays for skipping this with blame metadata on every constraint plus a 1,000-line module for naming `'1`; retrofitting blame onto a solver is exactly that mistake.
- **The nearest thing to a counterexample, and it isn't one:** design-1:1057's *"deduction binds `g` **exactly**, so mixed group arguments are a type error"* could be read as "unification must fail on mismatch," but a cardinality check at fold time produces the identical verdict and the doc doesn't pick between them.
- **Caveat, stated plainly:** absence of a counterexample in a prose spec is not a theorem. **If you hit a point mid-solve that needs to know which group, that is a finding worth reporting upstream** — it would contradict design-1:1118 and :1124 rather than merely surprising us.

**Effects are their own solver domain — NOT signature syntax.** The docs leave open whether `Effects` is a *kind* (groups get the tick `<g': Ship>` and effects get a plain `: Effects` bound; the attested `comptime <name>: <kind>` kinds are only `type` and `group`), but they rule out the alternative: design-1:1263 — *"the compiler unifies declared effects with derived effects; `!mut(path)` acts as a subtractive constraint on **effect variables**"* — plus design-1:1235 and :1237. A clause attached to a signature does not have variables that get solved, widened, and re-checked. So an `ITemplataT::Effect` beside `ITemplataT::Region` is consistent with everything written; treating effects as syntax is not.
- **Conformance constraint to encode early (design-1:1235, Pin C):** a negative bound is satisfied against the effect variable's **fully-solved value**, never against per-position contributions, and *"a solver widening `E` from a later position must re-check every negative bound that touches `E`."* An implementation that discharges `!mut` at the first position it sees is **non-conforming**.

**A group is an IDENTITY, not an extent.** This is the load-bearing fact and it was verified, not assumed. design-1's own section title is *"Groups are not lifetimes"*: *"A Rust lifetime delivers two guarantees through one mechanism — duration and exclusivity. Groups deliberately split these and drop exclusivity entirely… Group parameters on types are not validity durations: they name pointee sets."* `g in h` is defined as *"place-subset — set containment, **not** an outlives relation."* Consequences: **no outlives lattice, no variance machinery, no subtyping over groups** — everything is invariant, which is what makes group identity expressible in our existing templata/rune vocabulary at all. Do not import Rust's region reasoning; it answers a different question.

**What the call-site check actually is.** *Omission is a checked disjointness claim* (design-1 §Receivers): a signature's mutated groups include its effect targets, and omitting a declared relation between a bound group and an effect target **is** a disjointness claim, verified at every binding site. That is the whole of the per-call check.
- **Two args in one declared group is NOT an error** — the compiler *conjures a temporary union group* at the call site (`f(&game_x, &game_y)` binds `g = {game_x, game_y}`). Temp unions apply to plain borrow params only, never to group *arguments* of parameterized types (`Overlay<a>` vs `Overlay<b>` is a type error).
- Distinct owning fields are **provably disjoint with no declared relation** (the sibling-disjointness lemma), so `attack(&fleet.flagship, &fleet.escort)` needs no clause.

**Invalidation is keyed on REACH, not on `mut`.** design-1 defines reach as *"the union, over the op's effect targets, of the target's contents-territory — every destructible place beneath it, **excluding the target's own member places**."* This is why `attack` writing `attacker.fuel` invalidates nothing while `ships.clear()` kills every element borrow. A checker keyed on "a `mut` happened" rejects the motivating program.

**Two join disciplines, deliberately** (design-1 §"Which mode a join fact takes"): **monotone** facts (invalidation, poisoning, mention sets) are may-facts — union at joins, least fixpoint on loops. **Conserved** facts (move-state, linear obligations) admit no safe extreme, so a disagreeing join **rejects**. Our existing move tracker is the conserved kind; invalidation is the monotone kind. They sit side by side.

**The ladder** (each rung past 0 catches a distinct error class):
...
| 1 | effect clauses (`mut(g)`); the first phase-4 check | **disjointness violation** (declared disjoint, passed aliasing); permission escalation | **yes — file born here** |
| 2 | churn tracking | **use-after-churn**; borrow sibling to a churning receiver | yes, + monotone state |
| 3 | `Vec<T>` (rides the generics/bounds track) | same classes, realistic code | yes |

Rungs 0–2 need **no generics and no `Vec`** — plain structs suffice (`struct Fleet { flagship Ship; escort Ship; }`), so the borrow-checker track and the generic-bounds track are **parallel**, not sequential.

**PHASE 4 ALONE IS INSUFFICIENT — there are TWO seams.** `xs[i]` is a **place expression, not a call**: design-1:169 calls `&entity.rings[0]` *"a fresh borrow derived from a place"*; design-1:1332 names *"a claim-bearing element or field place"* with `self.units[i]`; design-1:902 and :936 bind group parameters **to** indexed places (`self_g ↦ tile.components[i]`, `self_g ↦ handlers[i]`); design-1:1147 makes `[]` a path segment unified across `Vec`/`List`/`Box`/arrays. The doc never mentions an `Index` trait or any indexing method.

So **borrow *creation* needs its own hook at the member/element-lookup seam**, and phase 4 at call sites covers borrow *checking* only. The two are genuinely separate in Valen, and they compose in a fixed order: design-1:898's dispatch shape `host.parts[i].method(host)` derives a receiver **from an indexed place**, and design-1:902 checks that binding at the call site — so the creation hook runs first and hands phase 4 something. Plan the file for both entry points from the start; this is exactly the "cheap now, expensive later" item.

**Quarantine by capability, not visibility.** The checker reads `ExpressionTE` (it must — a parallel fact-IR was considered and rejected: second representation, permanent sync cost, and rustc only gets away with MIR because MIR has four other consumers). The rule instead constrains what it may *do*: no `&mut CompilerOutputs`, no interner, no arena, no calls back into resolution; errors **returned**, the architect routes them. Entry point starts stateless — `check_call(callee_header, conclusions, arg_kinds) -> Result<(), BorrowError>` — and gains state only in the **I** step of the slice whose **R** step is the first use-after-churn test.

**The checker is PER-BODY, and the quarantine holds.** design-1:125 (resolution is symbolic at the typing pass), design-1:652 (poisoning computed per frame *"through the same channel — callees' effect clauses"*), and design-1:665's property: *"every use goes through a typed binding in some frame; each frame knows its own history; and handing a poisoned value to a callee is itself a use, so callees may assume their parameters arrive clean."* Nothing reads another body's *contents*.

**One precision on the interface: "signatures" means the WHOLE signature, not just the effect clause.** Two mechanisms need more — **`dangle` propagation** (design-1:713: a `dangle`-group reference may be handed only to callees that also accept it as `dangle`, so the callee's annotations are required) and the **relation-aware checks** (design-1:715 Pin F and design-1:1233: `dangle` violation and subtractive satisfaction both close over declared relations in scope — `maybealias`, `in`, and their closed forms — which live in **where-clauses**). So the read-only input is *effect clause + `dangle` annotations + where-clause relations*, not effects alone.

**Evidence from `~/rust` for these choices:**
- **~15,000 of borrowck's 30,750 lines exist because Rust *infers* lifetimes** — renumbering, universal-region discovery, the SCC constraint solver, blame metadata on every constraint, and a 1,000-line module (`diagnostics/region_name.rs`) devoted to inventing names for `'1`. With declared groups we skip all of it.
- **HIR typeck *erases* all regions on writeback, and borrowck re-typechecks the whole MIR body (~5,600 lines) purely to regenerate region constraints.** Strong evidence for keeping our checker in the typing pass where the group is already on the type.
- MIR was **edited for** borrowck after the fact (`FalseEdge`, `FakeRead`, special match lowering) to hide CFG structure from it — lowering to a CFG bought precision, then they spent effort blurring it back.
- rustc's structure is **two walks**: one location-local visitor with `check_call_inputs`/`check_call_dest` (exactly our phase 4), plus a body-global dataflow fixpoint. Our rung 2 is the second walk; plan the file so both coexist.
- `rustc_borrowck` is a **leaf crate** (two trivial reverse deps; everyone else goes through a query) — precedent that capability-quarantine works.
- Rust has **no declared disjointness** — it uses place-overlap detection (`places_conflict.rs`, ~526 lines). Our same-group-aliases-freely rule deletes that entirely.
- Two-phase borrows are a **desugaring artifact**, verified: `v.push(v.len())` compiles, the hand-desugared form is E0502. If we evaluate args before materializing the receiver borrow, the problem never arises.
- **The price of deferring regions is a second full type-check of the program.** HIR typeck builds its inference context `.ignoring_regions()`, so `'a: 'b` obligations are discharged as trivially true and writeback erases every region; MIR typeck then re-walks the whole body purely to regenerate constraints, and `bug!`s on any type error because HIR typeck already proved them. Quote the cost whenever quoting the design.
- **Cite rustc for the module boundary, never for the ordering.** `rustc_infer` does not depend on `rustc_trait_selection`, so unification genuinely cannot call selection — but the two engines then interleave through one shared store many times per body, and the obligation pool is per typeck-root rather than per call site.
