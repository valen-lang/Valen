...
### As a Real-Time Hook
Guardian runs automatically when Claude edits code (configured in `guardian.toml`). It checks each changed definition against active shields and blocks violations.

### CLI Commands
```bash
# Review changes against all shields
guardian review --config guardian.toml --mode review_mode --base HEAD --votes 1
