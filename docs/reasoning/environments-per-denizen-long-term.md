...
- `docs/reasoning/arena-deterministic-maps.md` — why arena-backed maps don't exist for us; part of why envs-in-arena forces slice-based `TemplatasStoreT`.
- `docs/reasoning/idt-typed-view-alternatives.md` — sister doc recording a similar deferred-alternatives decision for `IdT`.
...
- `docs/shields/ArenaTypesDontClone-ATDCX.md` — the shield that already carves out "working state (envs, solver types) may have `Clone`-without-`Copy`," anticipating the move to heap-storage working state.
