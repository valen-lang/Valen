...
The Scala source of truth is `Frontend/InstantiatingPass/src/dev/vale/instantiating/`. The Rust implementation lives in `src/instantiating/`.
...
The canonical driver in `src/pass_manager/pass_manager.rs` constructs bumps in this order:
...
All four use **`paste!`** (`::paste::paste!`) to construct the per-namespace method names (`[<$method _si>]`, `[<$method _ni>]`, `[<$method _ci>]`). The `paste` crate is a real build dependency (`Cargo.toml: paste = "1"`).
...
- `grep -n "ICompileErrorI"` over both `src/instantiating/` and `Frontend/InstantiatingPass/` returns zero hits. The type doesn't exist on either side.
...
| `src/instantiating/mod.rs` | 8 | module declarations |
| `src/instantiating/instantiating_arena.rs` | 20 | `InstantiatingArena<'i>` newtype |
| `src/instantiating/instantiating_interner.rs` | 836 | 12-family interner (4 kinds × 3 namespaces), ~216 macro-generated wrappers, `paste!`-based codegen |
| `src/instantiating/instantiator.rs` | 2717 | `InstantiatorI` god struct, `InstantiatedOutputsI`, 57 `translate_*` methods, `translate_method` driver |
| `src/instantiating/collector.rs` | 400 | `NodeRefI` visitor + `collect_*_inode!` macros |
| `src/instantiating/instantiated_compilation.rs` | 163 | 5-lifetime driver wrapper, lazy `monouts_cache` |
| `src/instantiating/instantiated_humanizer.rs` | 189 | I-side pretty-printer (sparse) |
| `src/instantiating/ast/mod.rs` | 11 | AST module declarations |
| `src/instantiating/ast/types.rs` | 304 | `CoordI`, `KindIT`, `SharednessI`, kind payloads, atom enums |
| `src/instantiating/ast/names.rs` | 1799 | `IdI`, `INameI` (72 variants), all `*NameI` structs, sub-enum families, TryFrom/From conversions |
| `src/instantiating/ast/ast.rs` | 420 | `PrototypeI`, `SignatureI`, `FunctionDefinitionI`, `FunctionHeaderI`, exports/externs, edges, attributes, local variables |
| `src/instantiating/ast/templata.rs` | 242 | `ITemplataI` (18 variants) + payload structs |
| `src/instantiating/ast/citizens.rs` | 113 | `StructDefinitionI`, `InterfaceDefinitionI`, `IMemberTypeI` |
| `src/instantiating/ast/expressions.rs` | 834 | `ExpressionIE`/`ReferenceExpressionIE`/`AddressExpressionIE` + ~50 payloads |
| `src/instantiating/ast/hinputs.rs` | 223 | `HinputsI`, `InstantiationBoundArgumentsI`, lookup methods |
| `src/pass_manager/pass_manager.rs` | | top-level driver; arena construction order |
...
- Delete the `paste = "1"` dependency from `Cargo.toml` — no per-suffix identifier construction needed anymore.
...
**1.2 Seal `IdI`.**
Bring `IdI` in line with its peers (`IdT`, `IdH`, all `*IT`/`PrototypeI`/`SignatureI`/`*NameI` types). Pattern to copy verbatim from `src/typing/names/names.rs` and `src/typing/typing_interner.rs`:
