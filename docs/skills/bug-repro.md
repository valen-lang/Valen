...
- **Place the minimal repro at the earliest pass that triggers it.** If the panic fires in postparsing, the test lives in `postparsing/test/`, not `pass_manager/end_to_end_test.rs`. If it fires in typing, `typing/test/`. End-to-end is the surface that surfaces the gap; the regression test belongs at the lowest layer that still reproduces it — narrower call graph, faster iteration, fewer downstream changes that could mask it. (Note: the retired `higher_typing` pass is no longer a valid target — regression fixtures that used to live at `higher_typing/tests/` are archived in `FrontendRust/docs/regression-fixtures-from-retired-higher-typing.md`.)
