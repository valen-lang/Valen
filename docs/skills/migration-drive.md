...
0. **All TL ↔ JR messaging goes through the `mailbox` binary** at `Luz/mailbox/target/release/mailbox` (build with `cargo build --manifest-path Luz/mailbox/Cargo.toml --release` if missing; see `Luz/mailbox/README.md`). Identities are derived from the worktree directory name: your own is `<dirname>-jr`, your TL is `<dirname>-tl` (preserve the dirname's casing). Compute via `ME="$(basename "$PWD")"` at the start of every mailbox command sequence; **never** hardcode `Vale-jr` / `Vale-tl` / any absolute path in the doc or in messages — it routes the wrong way when another worktree's session reads the doc. Always run `mailbox` from your worktree root so `--sender-cwd` defaults correctly and `--to-path .` routes to your own TL's inbox. Before starting a failing test, wait for TL to name it via mailbox. Before any substantive work, start a background `mailbox watch --as "${ME}-jr"` (snippet at step 5) so TL can interrupt you mid-task; **always re-arm a fresh watcher immediately after every `mailbox recv` (before doing any other work, in the same turn) — the watcher exits when it fires, so without immediate re-arming you are deaf to interrupts during the work you're about to do**.
...
5. Run the test again.
    ...
    * If it panics without "unimplemented"/"implement"/"not yet migrated" somewhere in the panic message, that's a **logic bug**, and it's not yours to fix. Mark the test `- [~]` in `migration-drive-todo.md` (change its `- [ ]` to `- [~]`) with a one-line note appended after the test name in the form `— BLOCKED: <rust_file:line> "<panic message>"`. Then send the same note to TL via `ME="$(basename "$PWD")" && mailbox send --as "${ME}-jr" --to "${ME}-tl" --to-path .` (where `<your-tl-id>` is the TL identifier for *your own worktree* — see step 0) so the TL queues it for later. Then wait for TL to name the next target via mailbox before un-ignoring anything. Don't try to fix logic bugs yourself.
    * If it passes, mark the test done in `migration-drive-todo.md` (change its `- [ ]` to `- [x]`). Run the full suite first and confirm no regressions. **Then notify TL via mailbox** (test name + `git diff --name-only` output + "ready to sync") and **wait for TL ack via `mailbox recv`** before un-ignoring the next test — see the "Sync after every green test" note below. Once TL names the next target, un-ignore it, write its Rust test body (using the Scala comment as a guide), and start driving it through the same loop. **Do not start on a new test until all currently-active tests are passing.**


Notes:

* **Sync after every green test.** When a test passes (step 3/5), `ME="$(basename "$PWD")" && mailbox send --as "${ME}-jr" --to "${ME}-tl" --to-path .` (where `<your-tl-id>` is the TL identifier for *your own worktree* — see step 0) with the test name, `git diff --name-only` output, and "ready to sync" — before picking the next `- [ ]`. TL handles the commit + CI rebase/ff into `experimental` (architect-gated). **Wait for TL ack via `mailbox recv --as "${ME}-jr"` before continuing.** Per-test sync is mandatory — don't batch multiple green tests before notifying. Run the full suite first to confirm no regressions; if regressions, fix them before notifying.

* **Send escalations via mailbox.** Whenever you escalate to the TL (scaffolding gap, SPDMX skeleton, lifetime error, choice between alternatives, anything in the bullets below that says "stop and escalate"), pipe the escalation body into `ME="$(basename "$PWD")" && mailbox send --as "${ME}-jr" --to "${ME}-tl" --to-path .` (where `<your-tl-id>` is the TL identifier for *your own worktree* — see step 0) rather than only mentioning it in chat. Include all the context the TL needs (file paths, line numbers, Scala counterpart, error message, options you considered). Chat is ephemeral; the archived mailbox file at `<tl-worktree>/tmp/messages/` is the source of truth. **When you escalate, stop — don't keep driving the test in parallel; wait for the TL response via your watcher + `mailbox recv`. Don't defer or skip a test for *scaffolding* gaps, *Guardian blocks*, or *lifetime errors* — those escalate-and-wait.** (Logic bugs are the exception, per step 5: those get `- [~]` and you move on.)

  **`send` will refuse with exit 2 if your inbox has any unread messages from anyone (including TL responses you haven't recv'd yet).** That's the race-safety feature — `mailbox recv --as "${ME}-jr"` first, then re-send. There is no `--force`.

  **After `mailbox send`-ing an escalation, kick off a background watcher so you're notified the moment TL responds, then yield the turn:**
  ```bash
  Luz/mailbox/target/release/mailbox watch --as "${ME}-jr"   # run from your worktree root
  ```
  Run that with `run_in_background: true` — `mailbox watch` polls cwd every 500ms for `from-*-to-jr-*.md`, **blocks until the first new arrival, prints its absolute path, and exits 0**. The harness fires its task-completion notification on process exit, so you wake up the moment TL's reply lands. When it fires: (1) `mailbox recv --as "${ME}-jr"` consumes the oldest unread (prints header + body, archives to `./tmp/messages/`); loop `recv` if multiple arrived. (2) **Immediately arm a fresh `mailbox watch --as "${ME}-jr"` before doing anything else** — the previous one already exited on the fire, so without re-arming RIGHT NOW you are deaf to a follow-up TL interrupt during whatever work you're about to do. (3) Then apply the instructions and continue driving.

* **When the architect says just "z," run `mailbox recv --as "${ME}-jr"`; if it prints a message, apply the TL's instructions and continue driving. If stdout is empty, nothing's pending — don't invent work.**
...
* **temp-disable:** Guardian will be running and watching any commands and edits. Pay attention to what it says, it's trying to keep things going in a good direction. However, if it's objectively wrong about something, or you feel an exception is extremely justified, then feel free to issue a temp-disable to temporarily turn off Guardian for a given definition. Temp-disables are issued by `curl`-POSTing to Guardian's local `/temp-disable` endpoint: `curl -s -X POST http://localhost:<PORT>/temp-disable -H 'Content-Type: application/json' -d '{"file_path": "...", "verdict_file": "...", "shield_file": "...", "reason": "...", "target_line": <N>}'` — `target_line` is required when the definition's name resolves to multiple defs in the file. The MCP-tool path was retired in favor of this direct-HTTP form.

* **Never put the literal substrings `/*` or `*/` into a Guardian directive's rationale text.** Both `Guardian: temp-disable: …` comments and `/* Guardian: ... */` blocks land adjacent to the trailing `/* Scala audit */` block, so a `/*` in your rationale forms a nested block comment that breaks SCPX parsing — and once that happens, NAGDX prevents you from editing the directive to repair it (only TL can unstick the file). When you need to quote Scala code in a rationale, paraphrase or cite by line range (e.g. `Scala at hamuts.rs:755-792`) instead of embedding the comment delimiters.
