...
* **Sync after every green test.** When a test passes (step 3/5), post to `from-jr.md` with the test name, `git diff --name-only` output, and "ready to sync" — before picking the next `- [ ]`. TL handles the commit + CI rebase/ff into `experimental` (architect-gated). **Wait for `for-jr.md` ack from TL before continuing.** Per-test sync is mandatory — don't batch multiple green tests before notifying. Run the full suite first to confirm no regressions; if regressions, fix them before notifying.

* **Write escalations to `from-jr.md`.** JR's outbound channel is `from-jr.md` at the repo root (sender-named). Whenever you escalate to the TL (scaffolding gap, SPDMX skeleton, lifetime error, choice between alternatives, anything in the bullets below that says "stop and escalate"), write the escalation into `from-jr.md` rather than only saying it in chat. Append a new section with a timestamp/heading, then include all the context the TL needs (file paths, line numbers, Scala counterpart, error message, options you considered). The TL reads `from-jr.md` to pick up escalations between turns. You can still mention in chat that you escalated, but `from-jr.md` is the source of truth — chat history is ephemeral, the file is not. **When you escalate, stop — don't keep driving the test in parallel; wait for the TL response in `for-jr.md`. Don't defer or skip a test for *scaffolding* gaps, *Guardian blocks*, or *lifetime errors* — those escalate-and-wait.** (Logic bugs are the exception, per step 5: those get `- [~]` and you move on.)
