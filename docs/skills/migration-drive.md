...
* **Missing `import foo.*` in tests: chain the existing resolvers, don't invent helpers.** `crate::builtins::builtins::get_embedded_modulized_code_map(...)` and `crate::tests::tests::get_package_to_resource_resolver()` already mirror Scala's two-resolver chain — `.or(...)` them in.
