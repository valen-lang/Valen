...
5. Run the test again.
    ...
    * If it panics without "unimplemented"/"implement"/"not yet migrated" somewhere in the panic message, that's a **logic bug**, and it's not yours to fix. Mark the test `- [~]` in `migration-drive-todo.md` (change its `- [ ]` to `- [~]`) with a one-line note appended after the test name in the form `— BLOCKED: <rust_file:line> "<panic message>"`. Then send the same note to TL via `mailbox send --as jr --to vale-tl --to-path /Volumes/V/Vale` so the TL queues it for later. Then wait for TL to name the next target via mailbox before un-ignoring anything. Don't try to fix logic bugs yourself.
