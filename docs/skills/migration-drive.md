...
5. Run the test again.
    ...
    * If it passes, mark the test done in `typing-test-todo.md` (change its `- [ ]` to `- [x]`), then pick the next `- [ ]` test in that file, un-ignore it in the test file, write its Rust test body (using the Scala comment as a guide), and start driving it through the same loop. **Do not start on a new test until all currently-active tests are passing** — run the full suite first and confirm no regressions before un-ignoring anything new.
