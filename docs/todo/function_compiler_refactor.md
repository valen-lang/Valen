...
1. **Tokens exist and thread through the main (non-macro) path.**
   ...
   * F: `cargo build --manifest-path Cargo.toml --lib` → expect failures at exactly those 7 sites plus their callers. Confirm the list matches before proceeding.
   ...
   * A: `cargo nextest run --manifest-path Cargo.toml`, then the same with `VALE_TEST_BACKEND=wasi`.
...
- Per-slice: `cargo build --manifest-path Cargo.toml --lib` (0 errors, **0 warnings** — the repo standard), then `cargo nextest run --manifest-path Cargo.toml`, then `VALE_TEST_BACKEND=wasi cargo nextest run --manifest-path Cargo.toml`. Both backends are the gate per `fire-commit-config.md`.
