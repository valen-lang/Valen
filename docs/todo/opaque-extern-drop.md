...
- `src/typing/macros/citizen/struct_drop_macro.rs` — the `SharednessT::Single if is_extern` branch. Currently panics with `auto-generated drop for extern struct is unsupported; supply an explicit extern func drop(...)`. When this design lands, this branch is replaced by extern-function-header synthesis (see below).
...
**Compile-time / Hammer-IR wire-format tests** (`src/integration_tests/tests/hammer_tests.rs`):
...
**Runtime end-to-end tests** (`src/integration_tests/tests/integration_tests_a.rs`):
