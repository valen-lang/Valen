...
Today we defined a comprehensive documentation strategy for the Sylvan project. The codebase has ~100+ markdown files scattered across `zen/`, `docs/`, `todo/`, `.claude/rules/`, `Luz/shields/`, and various `src/*/docs/` directories. Many overlap heavily (especially the 5 arena docs), some are stale, and there's no consistent structure.
...
```
src/<pass>/docs/
src/<pass>/docs/background/
src/<pass>/docs/usage/
src/<pass>/docs/arcana/
src/<pass>/docs/shields/
src/<pass>/docs/architecture/
src/<pass>/docs/reasoning/
```
...
### Source files
- `docs/arena-lifetimes.md` — the two arenas, what lives in each, data flow
- `docs/arena-two-phase-lifecycle.md` — build-mutable-then-freeze pattern
- `docs/arena-allocated-structs.md` — inventory of arena vs heap structs
- `docs/per-pass-arenas-migration.md` — plan to eliminate `'a`, per-pass design
- `zen/ArenaAndMallocSeparation.md` — principle: no malloc in arena structs
...
- `docs/location-in-denizen.md` — exemplifies arena-generic `'x` pattern
- `todo/arena-deterministic-map-problem.md` — unsolved problem within cluster

### Target structure
```
docs/background/arenas.md          — (#1) What the arenas are, lifetimes, what lives where
docs/usage/arenas.md                — (#2) How to allocate, build-then-freeze, conversions
docs/architecture/arenas.md         — (#6) Internal design, interning machinery, two-phase lifecycle details
docs/shields/ArenaAllocatedStructsShouldNotContainMallocdCollections-AASSNCMCX.md  — (#4) from zen/ArenaAndMallocSeparation.md
docs/reasoning/arena-deterministic-maps.md  — (#7) why we need deterministic arena maps, 5 approaches considered
```
...
## Phase 3: Sort zen/ (Day 2, ~2-3 hours)

`zen/` is being dissolved into `docs/`. Process each file per the `/document` skill.
...
- `zen/PostParserSynthesizesParserASTNodes-PPSPASTNZ.md` → `src/postparsing/docs/arcana/PostParserSynthesizesParserASTNodes-PPSPASTNZ.md`

### Architecture
- `zen/typing-pass-design.md` → `src/higher_typing/docs/architecture/typing-pass.md`

### After all moves, delete `zen/` directory
...
- `cursor_setup.md` → `docs/usage/cursor-setup.md` or delete if stale
- `thoughts.md` → review, likely delete or fold into reasoning
- `manual/building.md` → `docs/usage/building.md`
...
- `general/frontendrust-build-test.mdc` → `docs/usage/build-test.md`, symlink back
- `general/interning-patterns.mdc` → `docs/usage/interning.md`, symlink back
- `general/no-unsolicited-restructuring.mdc` → shield or keep as Claude-specific rule
- `general/style-guide.mdc` → `docs/usage/style.md`, symlink back
...
- `guardian-teach/SKILL.md`, `guardian-post-review/SKILL.md` → gets a brief entry in `docs/skills/` explaining the workflow, with the SKILL.md staying in `.claude/skills/` as the executable entrypoint.
...
ln -s ../../../src/postparsing/docs/usage/interning.md \
      .claude/rules/postparser/usage/interning.mdc

ln -s ../../../src/postparsing/docs/architecture/organization.md \
...
- `zen/` (all content moved)
- `todo/` (all content moved or deleted)

### Delete stale files
- `migrate-direction.md`
- `todo/necx-clone-analysis.md` (if NECX shield covers it)
