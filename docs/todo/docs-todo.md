...
- **`docs/`** — populated: 6 arcana, architecture/background/reasoning docs, 4 Sylvan-specific shields.
- **`zen/`** — **dissolved** (this pass): shield dupes deleted, `PPSPASTNZ` → `src/postparsing/docs/arcana/`, dead `NOT WORKING` python parity scripts + 77MB stale `logs/` removed.
...
Converted all 12 hand-written `.mdc`/`.md` rules into `docs/` docs with `g_read_when` + `g_auto_load_when_editing` frontmatter; `manifest-sync` now generates the `.claude/rules/*.mdc` (flat dashed names, project-root-relative globs). Homes: build-test, interning-patterns, style-guide → `docs/usage/`; solver/parser/postparser mappings, guidelines, organization, IDEPFL interning, differences → the respective `src/<pass>/docs/architecture/`.
...
- [ ] `todo.md` (kept per request)
- [x] `cursor_setup.md` — deleted
- [ ] `thoughts.md` — design scratchpad (Coord/region/ownership musings); decide reasoning-doc vs leave
- [ ] `manual/building.md` — 2-line build/test command ref; fold into `docs/usage/build-test.md` or leave
- [ ] `src/gripes.md` (leave per request)
...
- [ ] Spot-check that remaining `Luz/shields/` are genuinely project-agnostic; the Sylvan/arena-specific ones already moved to `docs/shields/` (AASSNCMCX, ATDCX, TFITCX).
