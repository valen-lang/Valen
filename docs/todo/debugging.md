...
2. The 120 squash-deferred `#[ignore]` tags can be lifted in a single sweep across `src/end_to_end_tests/tests/*.rs`.
...
- **Gate 1** at `src/backend_ffi/metal_lowerer.rs:178` — panics if `RangeS::internal` (synthetic range) reaches the FFI; replaces a silent `None` fallback.
