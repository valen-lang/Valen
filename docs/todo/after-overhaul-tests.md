...
- **3 bare `#[ignore]` with no documented reason** in `src/lexing/tests/lexing_iterator_tests.rs`: `peek_n_one_at_multibyte_boundary`, `chevron_comment_consumption_leaves_position_at_byte_index`, `testvale_full_file_parses`. Either annotate with a real justification or investigate whether they're still relevant.
...
`src/parsing/tests/functions/after_regions_function_tests.rs` is emptied on `experimental-1` because the lone parser-bug test was its only resident; the restore commit on `after-overhaul-tests` re-adds the full file.
