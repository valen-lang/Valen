<!-- session: b07b60f6-3f0b-4408-9d78-d2182f44f849; exported-bytes: 9737068; transcript: /Users/verdagon/.claude/projects/-Volumes-V-Vale2/b07b60f6-3f0b-4408-9d78-d2182f44f849.jsonl -->
...
**Suite 577 / 170 / 8**, postparse 88/88, warnings 8. Tree clean apart from the two convo exports just written — those are the only uncommitted files, and they'll want picking up in the next `fire commit` (same pattern as convo-6).

**Handoff updated** with the horizon plan you asked for — short term (finish the typing slice back to green), medium term (re-link `integration_tests/`, `instantiating/`, tier 1, source ranges), long term (rung 0 and the borrow checker, effects behind it, the backend arc) — each with its blocker named. Plus the full record of today's code work, the re-measured capability ladder, and three corrections to claims the file was carrying about itself.

**Conversation exported** as `convo-7-implements-restoration-and-parse-drain.md`, with the plan sibling.

## The session in one line each

The parse bucket went **50 → 1**, and none of it was what the handoff said it was — not stubs, not onion-independent, not decision-free. Making `parse_and_explore` report the error it had been discarding is what made that drainable in an afternoon.

`implements` turned out to be **collateral damage from a name-based sweep**, not a retirement — the plan had said *rename*. It came back as a declared bound rather than a rule, which left the solver untouched and needed one carrier where the old design needed two.

And the silent path Vale4 flagged was real: an exported non-shared struct's members were never checked. Reproduced with a test that said *"expected Err, got Ok"*, fixed, breaks nothing.
