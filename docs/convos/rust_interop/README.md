...
| 12 | `convo-12-lookup-paths-and-mechanism-ordering.md` | a lookup rule carries a path rather than a name, so two crates' same-named types both resolve; unrepresentable signatures decline instead of panicking; nested modules, re-exports and a composition case take the corpus 33 → 45. Two principles settled: uphold a property with the type system first and a source lint last, and the @NNGZ lint is dropped for failing loudly already. "Fence" retired as vocabulary. Three agents compare the qualified-name and multi-step-lookup designs; the path goes on the rule because every rune needs an `ITemplataType` and one fat rule materializes no intermediate |
