...
4. Do the instructions have multiple steps? If so, please pick the first one. We want the next step to bring over *the minimum* amount of Scala code that gets us *closer* to addressing what was going wrong.
   * If you see multiple panics causing teh current problem, only recommend fixing the one panic we actually hit (as reported by migrate-direction.md) or the one we would hit first. Do NOT recommend fixing multiple panics at once.
